...
static irqreturn_t iqs128_int_handler(void)
{
    ...
    irq_set_irq_type(iqs128_irq,
                     iqs128_int_value ? IRQ_TYPE_EDGE_FALLING : IRQ_TYPE_EDGE_RISING);
...
static int iqs128_probe(struct platform_device *dev)
{
    ...
    if (node) {
        ...
        ret = request_irq(iqs128_irq, (irq_handler_t)iqs128_int_handler,
                          mt_get_gpio_in(iqs128_eint_gpio | 0x80000000) ? IRQ_TYPE_EDGE_FALLING :
