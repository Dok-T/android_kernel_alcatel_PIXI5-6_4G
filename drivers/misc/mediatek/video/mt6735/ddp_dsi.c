...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			/*[FEATURE]-add-begin,add import log for esd function*/
			#ifdef CONFIG_PROJECT_PIXI5104G
			printk("jinghuang ESD [DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     read_data0.byte0, read_data0.byte1, read_data0.byte2,
			     read_data0.byte3);
			printk("jinghuang ESD [DSI]cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",
