...
kal_uint16 OV13858_R2A_write_i2c(u16 addr, u32 para)
{
	iWriteReg((u16) addr , (u32) para , 1, OV13858MIPI_WRITE_ID);
	return 1;
}

void ov13858_otp_cali(unsigned char writeid)
{
...
unsigned int ov13858_read_mid(unsigned char slave_addr)
{
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		...
		(*otp_ptr).rg_ratio = (OV13858_R2A_read_i2c(addr + 7)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13858_R2A_read_i2c(addr + 8)<<2) + ((temp>>4) & 0x03);	
