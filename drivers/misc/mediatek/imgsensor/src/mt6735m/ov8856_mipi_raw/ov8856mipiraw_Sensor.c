...
static imgsensor_info_struct imgsensor_info = { 
...
#ifndef USING_MAX_24FPS
		.framelength = 2482,			//record different mode's framelength
#else
		.framelength = 3092,			//record different mode's framelength
#endif
	...
	.sensor_mode_num = 5,	  //support sensor mode num ,don't support Slow motion
        ...
        //Modified by chengming.xiang for Task 4012335 On 2017-01-12 Begin	
        // Add capture delay frame to avoid Color error in HDR capture target at white and black object
	.cap_delay_frame = 2,		//enter capture delay frame num
        //Modified by chengming.xiang for Task 4012335 On 2017-01-12 End
...
struct otp_struct ov8858_otp_ptr ;
static int otp_initalize_flag = 0 ;
unsigned long g_u4AF_ov8856_INF;
unsigned long g_u4AF_ov8856_MACRO;
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	write_cmos_sensor(0x5001, 0x08 | temp1 );
	...
	addr = 0;
	LOG_INF("ov8856 %s :basic information otp_flag %d\n",__func__ ,otp_flag);
	...
	if(addr != 0) {
		(*otp_ptr).flag = 0x80; // valid info basic information in otp
	...
	addr = 0;
	LOG_INF("ov8856 %s :awb information otp_flag %d\n",__func__ ,otp_flag);
	if((otp_flag & 0xc0) == 0x40) {
	...
	if(addr != 0) {
		...
		(*otp_ptr).flag = (*otp_ptr).flag | 0x40 ;
...
#ifdef lkl
    printk("lkl ov8856 %s :vcm otp_flag %d\n",__func__ ,otp_flag);
#endif
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
...
/***add AF OTP function by kuanlong.liao at 20170311 start*****/
    g_u4AF_ov8856_INF=(* otp_ptr).VCM_start;
    g_u4AF_ov8856_MACRO=(* otp_ptr).VCM_end;
    printk("lkl ov8856 %s :(* otp_ptr).VCM_start=g_u4AF_INF=%d, (* otp_ptr).VCM_end=g_u4AF_MACRO=%d, (* otp_ptr).VCM_dir= %d\n",__func__ ,(* otp_ptr).VCM_start, (* otp_ptr).VCM_end, (* otp_ptr).VCM_dir);
/***add AF OTP function by kuanlong.liao at 20170311 end*****/

	// OTP Lenc Calibration
	otp_flag = read_cmos_sensor(0x702d);
	addr = 0;
        LOG_INF("ov8856 %s :lenc otp_flag %d\n",__func__ ,otp_flag);
	if((otp_flag & 0xc0) == 0x40) {
	...
	//set 0x5002[3] to ?1?
	temp1 = read_cmos_sensor(0x5001);
	write_cmos_sensor(0x5001, 0x08|(temp1 & (~0x08)));
	return (*otp_ptr).flag;
}
// return value:
// bit[7]: 0 no otp info, 1 valid otp info
...
static int apply_otp(void)
{
	...
	if(otp_initalize_flag == 0) {
		read_otp(&ov8858_otp_ptr);
	}
	
	LOG_INF("MID :%x %d-%d-%d ,rg_ratio: %d, bg_ratio: %d  otp_flag %d\n",
	(ov8858_otp_ptr).module_integrator_id , (ov8858_otp_ptr).production_year,(ov8858_otp_ptr).production_month ,(ov8858_otp_ptr).production_day ,
	(ov8858_otp_ptr).rg_ratio,(ov8858_otp_ptr).bg_ratio ,ov8858_otp_ptr.flag);
	   
	if ((ov8858_otp_ptr).flag & 0x40) {
		rg = (ov8858_otp_ptr).rg_ratio;
		bg = (ov8858_otp_ptr).bg_ratio;
	...
	if ((ov8858_otp_ptr).flag & 0x10) {
		LOG_INF("%s :lens shading vallid\n",__func__ );
		temp = read_cmos_sensor(0x5000);
		temp = 0x20 | temp;
		write_cmos_sensor(0x5000, temp);
		for(i=0;i<240;i++) {
			write_cmos_sensor(0x5900 + i, (ov8858_otp_ptr).lenc[i]);
		}
	}
	return (ov8858_otp_ptr).flag;
return 0;
}
...
/*alpha comment out for compile error ,write_shutter defined ,but not used*/
#if 0
...
/*alpha comment out for compile error ,gain2reg defined ,but not used*/
#if 0
...
#else
	write_cmos_sensor(0x380e, 0x0c); //0x04);
	write_cmos_sensor(0x380f, 0x14); //0xde);
#endif
	write_cmos_sensor(0x3810, 0x00);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			printk("OV8856 Read sensor id fail, i2c write id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id != sensor_id){
		printk("error: ov8856 read sensorID not match!\n");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
  ...
  //  unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    switch (feature_id) {
        case SENSOR_FEATURE_GET_PERIOD:
            ...
            *feature_para_len=4;
#ifdef lkl
    printk("lkl otp ov8856 add SENSOR_FEATURE_GET_PERIOD 3002");
#endif
            break;
...
#ifdef lkl
    printk("lkl SENSOR_FEATURE_SET_GAIN *feature_data=%lld\n",*feature_data);
#endif
            break;
...
#ifdef lkl
    printk("lkl otp test send command ");
#endif
                    break;
            }
			break;
