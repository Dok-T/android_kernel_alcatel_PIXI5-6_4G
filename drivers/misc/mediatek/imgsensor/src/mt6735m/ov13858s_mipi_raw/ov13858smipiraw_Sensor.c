...
#include "ov13858smipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV13858S_camera_sensor"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
#define LOG_1 LOG_INF("OV13858S,MIPI 4LANE\n")
...
//sensor otp
#ifdef OV13858S_OTP
    extern void ov13858s_otp_cali(unsigned char writeid);
#endif
extern unsigned int ov13858s_read_mid(unsigned char slave_addr);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13858S_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
    .checksum_value = 0xd6650427,//0xf86cfdf4,        //checksum value for Camera Auto Test
    .pre = {
        .pclk = 108000000,                //record different mode's pclk
        .linelength = 2244,                //record different mode's linelength
        // .framelength = 1608,            //record different mode's framelength
        .framelength = 2006,            //record different mode's framelength
    ...
    .cap1 = {                            
        ...
        .mipi_data_lp2hs_settle_dc = 100,//unit , ns
        /* 
         * less than 13M(include 13M),cap1 max framerate is 24fps,16M max 
         * framerate is 20fps, 20M max framerate is 15fps
         */
        .max_framerate = 150,
    },
    .normal_video = {
...
struct otp_struct ov13858_otp_ptr ;
static int otp_initalize_flag = 0 ;
// return value:
...
{
    int otp_flag, addr, temp, temp1 , i, checksum2 = 0;
    //set 0x5002[3] to ?0?
    otp_initalize_flag = 1 ;
    temp1 = read_cmos_sensor(0x5000);
    write_cmos_sensor(0x5000, (0x00 & 0x10) | ((~0x10) & temp1));
    // read OTP into buffer
    write_cmos_sensor(0x3d84, 0xC0);
    write_cmos_sensor(0x3d88, 0x72); // OTP start address
    write_cmos_sensor(0x3d89, 0x20);
    write_cmos_sensor(0x3d8A, 0x73); // OTP end address
    ...
    otp_flag = read_cmos_sensor(0x7220);
    addr = 0;
    LOG_INF("ov13858 %s :basic information otp_flag %d\n", __func__ , otp_flag);
    ...
    if(addr != 0) {
        (*otp_ptr).flag = 0xc0; // valid info basic information in otp
        (*otp_ptr).module_integrator_id = read_cmos_sensor(addr);
        (*otp_ptr).lens_id = read_cmos_sensor( addr + 1);
        (*otp_ptr).production_year = 2000 + read_cmos_sensor( addr + 4);
    ...
    temp = read_cmos_sensor(addr + 9);
    (*otp_ptr).rg_ratio = (read_cmos_sensor(addr + 7) << 2) + ((temp >> 6) & 0x03);
    (*otp_ptr).bg_ratio = (read_cmos_sensor(addr + 8) << 2) + ((temp >> 4) & 0x03);
    // OTP VCM Calibration
    otp_flag = read_cmos_sensor(0x73b8);
    LOG_INF("ov13858 %s :vcm otp_flag %d\n", __func__ , otp_flag);
    addr = 0;
    if((otp_flag & 0xc0) == 0x40) {
    ...
    otp_flag = read_cmos_sensor(0x723d);
    addr = 0;
    LOG_INF("ov13858 %s :lenc otp_flag %d\n", __func__ , otp_flag);
    if((otp_flag & 0xc0) == 0x40) {
    ...
    if(addr != 0) {
        ...
        (*otp_ptr).checksum = read_cmos_sensor(addr + 186);
    ...
    //set 0x5002[3] to ?1?
    temp1 = read_cmos_sensor(0x5000);
    write_cmos_sensor(0x5000, (0x10 & 0x10) | ((~0x10) & temp1));
    return (*otp_ptr).flag;
}
// return value:
// bit[7]: 0 no otp info, 1 valid otp info
...
{
    int RG_Ratio_Typical = 0x276, BG_Ratio_Typical = 0x2a6;
    int rg, bg, R_gain, G_gain, B_gain, Base_gain, temp, i;
    if(otp_initalize_flag == 0)
        read_otp(&ov13858_otp_ptr);
    LOG_INF("MID :%x %d-%d-%d ,rg_ratio: %d, bg_ratio: %d  otp_flag %d\n",
            (ov13858_otp_ptr).module_integrator_id , (ov13858_otp_ptr).production_year,
            (ov13858_otp_ptr).production_month , (ov13858_otp_ptr).production_day ,
            (ov13858_otp_ptr).rg_ratio, (ov13858_otp_ptr).bg_ratio , ov13858_otp_ptr.flag);
    if ((ov13858_otp_ptr).flag & 0x40) {
        rg = (ov13858_otp_ptr).rg_ratio;
        bg = (ov13858_otp_ptr).bg_ratio;
        if(rg == 0) rg = RG_Ratio_Typical ;
        if(bg == 0) bg = BG_Ratio_Typical ;
        //calculate G gain
        ...
        if (R_gain > 0x400) {
            write_cmos_sensor(0x5104, R_gain >> 8);
            write_cmos_sensor(0x5105, R_gain & 0x00ff);
        }
        if (G_gain > 0x400) {
            write_cmos_sensor(0x5102, G_gain >> 8);
            write_cmos_sensor(0x5103, G_gain & 0x00ff);
        }
        if (B_gain > 0x400) {
            write_cmos_sensor(0x5100, B_gain >> 8);
            write_cmos_sensor(0x5101, B_gain & 0x00ff);
        }
    }
    // apply OTP Lenc Calibration
    if ((ov13858_otp_ptr).flag & 0x10) {
        LOG_INF("%s :lens shading vallid\n", __func__ );
        temp = read_cmos_sensor(0x5000);
        temp = 0x04 | temp;
        write_cmos_sensor(0x5000, temp);
        for(i = 0; i < 186; i++)
            write_cmos_sensor(0x5500 + i, (ov13858_otp_ptr).lenc[i]);
    }
    return (ov13858_otp_ptr).flag;
    return 0;
}
...
static kal_uint32 return_sensor_id(void)
{
	...
	id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 3;

	if (id == OV13858_SENSOR_ID){
		write_cmos_sensor(0x0100, 0x01);
		mid = ov13858s_read_mid(imgsensor.i2c_write_id);
		write_cmos_sensor(0x0100, 0x00);
		LOG_INF("OV13858S Module Integrator ID = 0x%x\n", mid);
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    if (imgsensor.autoflicker_en) {
        ...
        // if(realtime_fps >= 277 && realtime_fps <= 285)
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
    ...
    switch (image_mirror) {
    case IMAGE_NORMAL:
        write_cmos_sensor(0x3820, ((read_cmos_sensor(0x3820) & 0xe7) | 0x08));
        //write_cmos_sensor(0x3821,((read_cmos_sensor(0x3821) & 0xFB) | 0x00));
        break;
    case IMAGE_H_MIRROR:
        write_cmos_sensor(0x3820, ((read_cmos_sensor(0x3820) & 0xe7) | 0x00));
        //write_cmos_sensor(0x3821,((read_cmos_sensor(0x3821) & 0xFF) | 0x04));
        break;
    case IMAGE_V_MIRROR:
        write_cmos_sensor(0x3820, ((read_cmos_sensor(0x3820) & 0xe7) | 0x18));
        //write_cmos_sensor(0x3821,((read_cmos_sensor(0x3821) & 0xFB) | 0x00));
        break;
    case IMAGE_HV_MIRROR:
        write_cmos_sensor(0x3820, ((read_cmos_sensor(0x3820) & 0xe7) | 0x10));
        //write_cmos_sensor(0x3821,((read_cmos_sensor(0x3821) & 0xFF) | 0x04));
        break;
    default:
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x5000, 0xfd); //0xff); for 13858, Kencui@09/27
    write_cmos_sensor(0x5001, 0x05); //0x07);
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x380c, 0x08);//0x04);
    write_cmos_sensor(0x380d, 0xc4);//0x62);
    write_cmos_sensor(0x380e, 0x07); //0x0c);//6
    write_cmos_sensor(0x380f, 0xd6); //0x8e);//48
    ...
    write_cmos_sensor(0x4050, 0x04);
    write_cmos_sensor(0x4051, 0x0b);
    write_cmos_sensor(0x4837, 0x1e);//1c
    write_cmos_sensor(0x4902, 0x01);
    write_cmos_sensor(0x0100, 0x01);
    mdelay(10);
}    /*    preview_setting  */
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    if (capture_first_flag == 0) {
        if (currefps == 240) { //300//30fps            //30fps for Normal capture & ZSD
            //full size @ 30fps
            write_cmos_sensor(0x0100, 0x00);
            write_cmos_sensor(0x0303, 0x00);
            //write_cmos_sensor(0x3665, 0xa7);// just testing
            write_cmos_sensor(0x3501, 0x80);
            write_cmos_sensor(0x3662, 0x12);
            write_cmos_sensor(0x3714, 0x24);
            write_cmos_sensor(0x3737, 0x04);
            ...
            write_cmos_sensor(0x380a, 0x0c);
            write_cmos_sensor(0x380b, 0x40);
            write_cmos_sensor(0x380c, 0x04);//0x04);
            write_cmos_sensor(0x380d, 0xb0);//0x62);
            write_cmos_sensor(0x380e, 0x0e);//0c
            write_cmos_sensor(0x380f, 0xa6);//8e
            write_cmos_sensor(0x3811, 0x10);
            write_cmos_sensor(0x3813, 0x08);
            write_cmos_sensor(0x3814, 0x01);
            write_cmos_sensor(0x3816, 0x01);
...
static void slim_video_setting(void)
{
    ...
    write_cmos_sensor(0x380c, 0x08);//0x04);
    write_cmos_sensor(0x380d, 0xc4);//0x62);
    write_cmos_sensor(0x380e, 0x06);//0x0c);
    write_cmos_sensor(0x380f, 0x48);//0x8e);
    write_cmos_sensor(0x3811, 0x08);
    write_cmos_sensor(0x3813, 0x02);
    write_cmos_sensor(0x3814, 0x03);
    write_cmos_sensor(0x3816, 0x03);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if (enable) {
        ...
        // 0x5E00[8]: 1 enable,  0 disable
        // 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
        write_cmos_sensor(0x5000, 0xfd); //0xff);//enable otp_dpc
        write_cmos_sensor(0x4503, 0x00);
    }
    spin_lock(&imgsensor_drv_lock);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            pr_err("ov13858 Read sensor id fail, write id: 0x%x, id: 0x%x\n",
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            pr_err("ov13858s Read sensor id fail, write id: 0x%x, id: 0x%x\n",
                   imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while(retry > 0);
        i++;
    ...
    sensor_init();
#ifdef OV13858S_OTP
    ov13858s_otp_cali(imgsensor.i2c_write_id);
    write_cmos_sensor(0x0100, 0x00);
#endif
    spin_lock(&imgsensor_drv_lock);
...
UINT32 OV13858S_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
