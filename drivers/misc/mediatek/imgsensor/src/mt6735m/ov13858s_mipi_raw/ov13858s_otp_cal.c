...
static kal_uint16 OV13858_R2A_read_i2c(kal_uint32 addr)
{
...
static kal_uint16 OV13858_R2A_write_i2c(u16 addr, u32 para)
{
...
EXPORT_SYMBOL(ov13858s_otp_cali);


unsigned int ov13858s_read_mid(unsigned char slave_addr)
{
    unsigned int mid = 0x00;
    int otp_flag = 0, addr = 0, temp = 0;

    OV13858MIPI_WRITE_ID = slave_addr;
    temp = OV13858_R2A_read_i2c(0x5000);
    OV13858_R2A_write_i2c(0x5000, (0x00 & 0x10) | (temp & (~0x10)));
    ...
    if(addr != 0) 
        mid = OV13858_R2A_read_i2c(addr);

    temp = 0x00;
    temp = OV13858_R2A_read_i2c(0x5000);
    OV13858_R2A_write_i2c(0x5000, (0x10 & 0x10) | (temp & (~0x10)));

    OV13858MIPI_WRITE_ID = 0X00;

    return mid;
}
EXPORT_SYMBOL(ov13858_read_mid);
...
static int read_otp(struct otp_struct *otp_ptr)
{
    ...
    if(addr != 0) {
        ...
        (*otp_ptr).production_year = OV13858_R2A_read_i2c( addr + 4);
        (*otp_ptr).production_month = OV13858_R2A_read_i2c( addr + 5);
        (*otp_ptr).production_day = OV13858_R2A_read_i2c(addr + 6);
        temp = OV13858_R2A_read_i2c(addr + 9);
        (*otp_ptr).rg_ratio = (OV13858_R2A_read_i2c(addr + 7) << 2) + ((
                                  temp >> 6) & 0x03);
        (*otp_ptr).bg_ratio = (OV13858_R2A_read_i2c(addr + 8) << 2) + ((
                                  temp >> 4) & 0x03);
        LOG_INF("cuckoo r/g: %d  b/g: %d\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
    } else {
    ...
    // OTP VCM Calibration
    otp_flag = OV13858_R2A_read_i2c(0x73b8);
    ...
    if(addr != 0) {
        ...
        (* otp_ptr).VCM_start = (OV13858_R2A_read_i2c(addr) << 2) | ((
                                    temp >> 6) & 0x03);
        (* otp_ptr).VCM_end = (OV13858_R2A_read_i2c(addr + 1) << 2) | ((
                                  temp >> 4) & 0x03);
    ...
    otp_flag = OV13858_R2A_read_i2c(0x723d);
    ...
    if(addr != 0) {
        ...
        LOG_INF(" checksumLSC-OTP-Total:%x-%x-%x;read checksumLSC-OTP-Total:%x-%x-%x\n",
                checksumLSC, checksumOTP, checksumTotal, (* otp_ptr).checksumLSC,
                (* otp_ptr).checksumOTP, (* otp_ptr).checksumTotal);
...
static void LumaDecoder(uint8_t *pData, uint8_t *pPara)
{
    ...
    if(Bit <= 5) {
        for(i = 0, k = 2; i < 120; i += 8, k += 5) {
            ...
            pPara[i + 1] = ((pData[k] & 0x7) << 2) | (pData[k + 1] >>
                           6); // 2~0 (byte0) and 7~6(byte1)
            pPara[i + 2] = (pData[k + 1] & 0x3e) >> 1; // 5~1 (byte1)
            pPara[i + 3] = ((pData[k + 1] & 0x1) << 4) | (pData[k + 2] >>
                           4); // 0 (byte1) and 7~4(byte2)
            pPara[i + 4] = ((pData[k + 2] & 0xf) << 1) | (pData[k + 3] >>
                           7); // 3~0 (byte2) and 7(byte3)
            pPara[i + 5] = (pData[k + 3] & 0x7c) >> 2; // 6~2 (byte3)
            pPara[i + 6] = ((pData[k + 3] & 0x3) << 3) | (pData[k + 4] >>
                           5); // 1~0 (byte3) and 7~5(byte4)
        ...
        for(i = 48, k = 32; i < 120; i += 4, k += 3) {
            ...
            pPara[i + 1] = ((pData[k] & 0x3) << 4) | (pData[k + 1] >>
                           4); //1~0 (byte0) and7~4(byte1)
            pPara[i + 2] = ((pData[k + 1] & 0xf) << 2) | (pData[k + 2] >>
                           6); //3~0 (byte1) and7~6(byte2)
...
static void ColorDecoder(uint8_t *pData, uint8_t *pPara)
{
    ...
    pPara[1] = ((pData[1] & 0x1) << 4) | (pData[2] >>
                                          4); // 0 (byte1) and 7~4 (byte2)
    pPara[2] = ((pData[2] & 0xf) << 1) | (pData[3] >>
                                          7); // 3~0 (byte2) and 7 (byte3)
    pPara[3] = (pData[3] & 0x7c) >> 2; // 6~2 (byte3)
    pPara[4] = ((pData[3] & 0x3) << 3) | (pData[4] >>
                                          5); // 1~0 (byte3) and 7~5 (byte4)
