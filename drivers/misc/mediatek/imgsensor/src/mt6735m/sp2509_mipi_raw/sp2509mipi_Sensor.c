...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP2509_RAW_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
		     printk("imgsensor.i2c_write_id=%x\n",imgsensor.i2c_write_id);
        do {
            *sensor_id = return_sensor_id();
			     printk("sensor_id=%x\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("sp2509 i2c write id: 0x%x, *sensor_id == imgsensor_info.sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("sp2509 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("sp2509 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("sp2509 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
