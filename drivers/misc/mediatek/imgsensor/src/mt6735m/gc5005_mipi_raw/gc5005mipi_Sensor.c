...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .framelength = 2980,            //record different mode's framelength
...
#define RG_TYPICAL    		        0x02a5
#define BG_TYPICAL			0x0263
#define INFO_ROM_START			0x01
#define INFO_WIDTH       			0x07
#define WB_ROM_START           		0x11
#define WB_WIDTH         			0x0b
#define GOLDEN_ROM_START           	0x16
#define GOLDEN_WIDTH         		0x0b
...
//DD
	...
	flag_golden = gc5005_read_otp(0xf);
...
//INFO&WB
	for(index=0;index<2;index++)
	{
		...
		switch((flag_golden>>(6 - 2 * index))&0x03)
		{
		case 0x00:
		...
		case 0x01:
			...
			gc5005_otp_info.rg_gain = gc5005_read_otp(wb_start_add)<<8 | gc5005_read_otp(wb_start_add + 1);
			gc5005_otp_info.bg_gain = gc5005_read_otp(wb_start_add + 2)<<8 | gc5005_read_otp(wb_start_add + 3); 
			LOG_INF("GC5005_OTP_WB rg gain = %d bg gain = %d\n",gc5005_otp_info.rg_gain,gc5005_otp_info.bg_gain);
		...
		switch((flag_golden>>(6 - 2 * index))&0x03)
	{
		...
		case 0x01:
			...
			gc5005_otp_info.golden_rg= gc5005_read_otp(golden_start_add)<<8 | gc5005_read_otp(golden_start_add + 1);
			gc5005_otp_info.golden_bg = gc5005_read_otp(golden_start_add + 2)<<8 | gc5005_read_otp(golden_start_add + 3); 
			pr_err("GC5005_OTP_GOLDEN grg gain = %d gbg gain = %d\n",gc5005_otp_info.golden_rg,gc5005_otp_info.golden_bg);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x07, 0x03);                                                                                                         
	write_cmos_sensor(0x08, 0xec);                                                                                                         
