...
#define HI553_OTP_FUNCTION 1//Hi-553 OPT fuction begin
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.framelength = 3125,			//record different mode's framelength
...
#if HI553_OTP_FUNCTION //Hi-553 OPT fuction begin
#define RG_Ratio_Typical (0x152)
...
static kal_uint8 HI553_OTP_read_cmos_sensor(kal_uint32 addr)
{
...
void HI553OTPSetting(void)
{
  HI553_OTP_write_cmos_sensor(0x0A02, 0x01); //Fast sleep on
  HI553_OTP_write_cmos_sensor(0x0A00, 0x00);//stand by on
  mdelay(10);
  HI553_OTP_write_cmos_sensor(0x0f02, 0x00);//pll disable
  HI553_OTP_write_cmos_sensor(0x011a, 0x01);//CP TRIM_H
  HI553_OTP_write_cmos_sensor(0x011b, 0x09);//IPGM TRIM_H
  HI553_OTP_write_cmos_sensor(0x0d04, 0x01);//Fsync(OTP busy)Output Enable 
  HI553_OTP_write_cmos_sensor(0x0d00, 0x07);//Fsync(OTP busy)Output Drivability
  HI553_OTP_write_cmos_sensor(0x003f, 0x10);//OTP r/w mode
  HI553_OTP_write_cmos_sensor(0x0a00, 0x01);//standby off
}


void HI553OTPSettingDeinit(void)
{
  HI553_OTP_write_cmos_sensor(0x0A00, 0x00);//stand by on
  mdelay(10);
  HI553_OTP_write_cmos_sensor(0x003f, 0x00);//OTP r/w mode
  HI553_OTP_write_cmos_sensor(0x0A00, 0x01);//stand by on
}

kal_uint8 HI553_Sensor_OTP_read(kal_uint16 otp_addr)
{
    kal_uint16 i;
    kal_uint8 data;
  i = otp_addr;

    HI553_OTP_write_cmos_sensor(0x010a, (i >> 8) & 0xFF); //start address H
    HI553_OTP_write_cmos_sensor(0x010b, i & 0xFF); //start address L
    HI553_OTP_write_cmos_sensor(0x0102, 0x01); //read enable
    data = HI553_OTP_read_cmos_sensor(0x0108); //OTP data read
  return data;
}
...
void Hi553_Sensor_OTP_info(void)
{
  ...
  LOG_INF("%s enter\n",__func__);
      //info_flag = HI553_Sensor_OTP_read(0xBCEA);
   if (HI553_Sensor_OTP_read(0x0501) == 0x01)
	   {info_flag = 1;}
   else if (HI553_Sensor_OTP_read(0x0501) == 0x13)
	   {info_flag = 2;}
   else if (HI553_Sensor_OTP_read(0x0501) == 0x37)
	   {info_flag = 3;}
  LOG_INF("%s,info_flag = 0x%x\n",__func__,info_flag);
    switch (info_flag)
  {
    case 0x01:  //Group1
    ModuleHouseID = HI553_Sensor_OTP_read(0x0502);
	AFFF = HI553_Sensor_OTP_read(0x0503);
    Year = HI553_Sensor_OTP_read(0x0504);
    Month = HI553_Sensor_OTP_read(0x0505);
    Day = HI553_Sensor_OTP_read(0x0506);
    SensorID = HI553_Sensor_OTP_read(0x0507);
    LensID = HI553_Sensor_OTP_read(0x0508);
    VCMID = HI553_Sensor_OTP_read(0x0509);
    DriverID = HI553_Sensor_OTP_read(0x050a);
    //FNoID = HI553_Sensor_OTP_read(0x050b);
    infocheck = HI553_Sensor_OTP_read(0x0512);   //020f  20160524
      break;
    case 0x02:
    ModuleHouseID = HI553_Sensor_OTP_read(0x0513);
    AFFF = HI553_Sensor_OTP_read(0x0514);
    Year = HI553_Sensor_OTP_read(0x0515);
    Month = HI553_Sensor_OTP_read(0x0516);
    Day = HI553_Sensor_OTP_read(0x0517);
    SensorID = HI553_Sensor_OTP_read(0x0518);
    LensID = HI553_Sensor_OTP_read(0x0519);
    VCMID = HI553_Sensor_OTP_read(0x051a);
    DriverID = HI553_Sensor_OTP_read(0x051b);
    //FNoID = HI553_Sensor_OTP_read(0x051c);
    infocheck = HI553_Sensor_OTP_read(0x0523);     //0220  20160524
    break;
    case 0x03:
    ModuleHouseID = HI553_Sensor_OTP_read(0x0524);
    AFFF = HI553_Sensor_OTP_read(0x0525);
    Year = HI553_Sensor_OTP_read(0x0526);
    Month = HI553_Sensor_OTP_read(0x0527);
    Day = HI553_Sensor_OTP_read(0x0528);
    SensorID = HI553_Sensor_OTP_read(0x0529);
    LensID = HI553_Sensor_OTP_read(0x052a);
    VCMID = HI553_Sensor_OTP_read(0x052b);
    DriverID = HI553_Sensor_OTP_read(0x052c);
    //FNoID = HI553_Sensor_OTP_read(0x022d);
    infocheck = HI553_Sensor_OTP_read(0x0534);     //0231  20160524
    break;
    default:
	...
	checksum = (ModuleHouseID + AFFF + Year + Month + Day + SensorID + LensID + VCMID + DriverID) % 255+1;
...
void HI553_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain)
{


    HI553_OTP_write_cmos_sensor(0x0126, g_gain >> 8); //gr_gain
    HI553_OTP_write_cmos_sensor(0x0127, g_gain & 0xFF); //gr_gain
    HI553_OTP_write_cmos_sensor(0x0128, g_gain >> 8); //gb_gain
    HI553_OTP_write_cmos_sensor(0x0129, g_gain & 0xFF); //gb_gain

    HI553_OTP_write_cmos_sensor(0x012a, r_gain >> 8); //r_gain
    HI553_OTP_write_cmos_sensor(0x012b, r_gain & 0xFF); //r_gain
    HI553_OTP_write_cmos_sensor(0x012c, b_gain >> 8); //b_gain
    HI553_OTP_write_cmos_sensor(0x012d, b_gain & 0xFF); //b_gain
...
void HI553_Sensor_calc_wbdata(void)
{
	...
	LOG_INF("%s enter\n",__func__);
	wb_flag = HI553_Sensor_OTP_read(0x0535);
	LOG_INF("wb_flag = 0x%x %s\n",wb_flag,__func__);
	if (HI553_Sensor_OTP_read(0x0535) == 0x01)
	{
		wb_flag = 1;
	}
	else if (HI553_Sensor_OTP_read(0x0535) == 0x13)
	{
		wb_flag = 2;
	}
	else if (HI553_Sensor_OTP_read(0x0535) == 0x37)
	{
		wb_flag = 3;
	}
	...
	switch (wb_flag)
	{
		case 0x01:  //Group1 valid
			wb_unit_rg_h = HI553_Sensor_OTP_read(0x0536);
			wb_unit_rg_l = HI553_Sensor_OTP_read(0x0537);
			wb_unit_bg_h = HI553_Sensor_OTP_read(0x0538);
			wb_unit_bg_l = HI553_Sensor_OTP_read(0x0539);
			wb_unit_gg_h = HI553_Sensor_OTP_read(0x053a);
			wb_unit_gg_l = HI553_Sensor_OTP_read(0x053b);
			wbcheck = HI553_Sensor_OTP_read(0x0553);

			break;

		case 0x02:  //Group2 valid
			wb_unit_rg_h = HI553_Sensor_OTP_read(0x0554);
			wb_unit_rg_l = HI553_Sensor_OTP_read(0x0555);
			wb_unit_bg_h = HI553_Sensor_OTP_read(0x0556);
			wb_unit_bg_l = HI553_Sensor_OTP_read(0x0557);
			wb_unit_gg_h = HI553_Sensor_OTP_read(0x0558);
			wb_unit_gg_l = HI553_Sensor_OTP_read(0x0559);
			wbcheck = HI553_Sensor_OTP_read(0x0571);

			break;
		case 0x03:  //Group3 valid
			wb_unit_rg_h = HI553_Sensor_OTP_read(0x0572);
			wb_unit_rg_l = HI553_Sensor_OTP_read(0x0573);
			wb_unit_bg_h = HI553_Sensor_OTP_read(0x0574);
			wb_unit_bg_l = HI553_Sensor_OTP_read(0x0575);
			wb_unit_gg_h = HI553_Sensor_OTP_read(0x0576);
			wb_unit_gg_l = HI553_Sensor_OTP_read(0x0577);
			wbcheck = HI553_Sensor_OTP_read(0x058f);
	...
	if ((rg_value!=0) && (bg_value!=0)){
		r_gain = (RG_Ratio_Typical * 0x100) / rg_value;
		b_gain = (BG_Ratio_Typical * 0x100) / bg_value;
	}
	...
	HI553_Sensor_update_wb_gain(r_gain, g_gain,b_gain);
}
...
////////////////////////////////////////////////
	...
	write_cmos_sensor(0x0006, 0x0c35); //frame length lines
