...
static u32 charging_hw_init(void *data)
{
	...
	fan5405_reg_config_interface(0x06, 0x7A);
	...
	if (!charging_init_flag) {
		fan5405_reg_config_interface(0x04, 0x3B);	/* 192mA */
...
//		printk(KERN_ERR"############current_value=%d set_chr_current=%d",current_value,set_chr_current);
		register_value = charging_parameter_to_value(CS_VTH, array_size, set_chr_current);
//		printk(KERN_ERR"############register_value=%d",register_value);
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20170217,PR4224688
//use default SOC 50%,when battery ntc/id not exsit in MINI TEST
	...
	*(kal_bool *) (data) = 0;	/* battery exist */
	...
	//if (val) {
		...
		*(kal_bool *) (data) = pmic_get_register_value(PMIC_RGS_BATON_UNDET);
		battery_log(BAT_LOG_FULL, "[charging_get_battery_status] bat is not exist = %d\n", *(kal_bool *) (data));
	//} else {
	//	*(kal_bool *) (data) = KAL_FALSE;
	//}
#endif
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20170217,PR4224688
...
static u32 charging_get_fault_state(void *date)
{
	u32 status = STATUS_OK;
	*(unsigned int *) (date)=fan5405_get_fault_status();
	return status;
}
