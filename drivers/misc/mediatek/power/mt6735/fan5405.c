...
{
	int i = 0;
	unsigned char monitor_reg = 0;

	battery_log(BAT_LOG_FULL, "[fan5405] ");
	for (i = 0; i < fan5405_REG_NUM; i++) {
		fan5405_read_byte(i, &fan5405_reg[i]);
		battery_log(BAT_LOG_FULL, "[0x%x]=0x%x \n", i, fan5405_reg[i]);
	}
	fan5405_read_byte(16,&monitor_reg);
	
	battery_log(BAT_LOG_FULL,"[0x10]=0x%x \n",monitor_reg);
}
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.wenlong.huang@tcl.com,11/23/2016
static ssize_t show_fan5405_safeV(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned char reg6 = 0;	
	unsigned int v = 0;
	fan5405_read_byte(0x06,&reg6);
	switch(reg6 & 0x0f){
	case 0: 
	    	v = 420; 
		break;
...
static DEVICE_ATTR(fan5405_safeV, 0664, show_fan5405_safeV, NULL);	/* 664 */
//[BUGFIX]-Mod-END by SCDTABLET.wenlong.huang@tcl.com,11/23/2016
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.wenlong.huang@tcl.com,11/23/2016
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_fan5405_safeV);
//[BUGFIX]-Mod-END by SCDTABLET.wenlong.huang@tcl.com,11/23/2016
