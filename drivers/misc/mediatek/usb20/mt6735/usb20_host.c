...
#include <linux/io.h>
#if 0
#ifndef CONFIG_PROJECT_PIXI5104G
...
static bool musb_is_host(void)
{
#if 0
#ifndef CONFIG_PROJECT_PIXI5104G
	static bool otg_first_time = true;
	int ret = 0;
#endif
#endif
	u8 devctl = 0;
	...
	#endif
	DBG(0, "iddig_state = %d\n", iddig_state);
//[BUGFIX]-ADD-BEGIN by bao.li@tcl.com,2016/11/30,alm 3634789
#if 0
#ifndef CONFIG_PROJECT_PIXI5104G
	if (iddig_state == 0 && otg_first_time == true) {

		ret = mt_set_gpio_mode(GPIO_CHARGE_LED_PIN, GPIO_MODE_00);
		if (0 != ret)
			printk("[otg]GPIO_CHARGE_LED_PIN mt_set_gpio_mode fail\n");

		ret = mt_set_gpio_dir(GPIO_CHARGE_LED_PIN, GPIO_DIR_OUT);
		if (0 != ret)
			printk("[otg]GPIO_CHARGE_LED_PIN mt_set_gpio_dir fail\n");

		ret = mt_set_gpio_out(GPIO_CHARGE_LED_PIN, GPIO_OUT_ONE);
		printk("[otg]GPIO_CHARGE_LED_PIN mt_set_gpio_out =%d\n",ret);

		ret = mt_get_gpio_out(GPIO_CHARGE_LED_PIN);
		printk("[otg]GPIO_CHARGE_LED_PIN the gpio state =%d\n",ret);

		otg_first_time = false;
	}
#endif
#endif
