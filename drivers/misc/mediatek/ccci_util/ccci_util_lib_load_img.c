...
#endif
//[BUGFIX]-Add-BEGIN by SCDTABLET.lifang.cheng,12/12/2016,3510383,
//add feature multi-MD
#include <mt_gpio.h>
//[BUGFIX]-Add-END by SCDTABLET.lifang.cheng
...
//[BUGFIX]-Add-BEGIN by SCDTABLET.lifang.cheng,01/11/2017,3585289,
//get board id from command line
#ifdef CONFIG_MULTI_MD
void get_board_id(int md_id,unsigned char *board_id)
{
        char *ptr_cmdline = NULL;
        char ch;
        ptr_cmdline = strstr(saved_command_line,"board_id=");

	if(NULL!=ptr_cmdline)
	{
	        ch = ptr_cmdline[strlen("board_id=")];

		if((ch>='0')&&(ch<='9'))
		{
		   *board_id = ch -'0';
		  CCCI_UTIL_ERR_MSG_WITH_ID(md_id, "get_md_postfix:board_id=%d\n",*board_id);
		}
	}
	else
	{
		CCCI_UTIL_ERR_MSG_WITH_ID(md_id, "get_md_postfix:can't find board id in command line\n");
	}
}
/**********************************************/
//board id for pixi564
//gpio95    gpio90    gpio122  modem_image_id    hw_version
//  0               0         0                          1                  EU1S                                         
//  0               0         1                          1                  EU2S  
//  0               1         0                          3                  LATAM1S  
//  0               1         1                          3                  LATAM2S 
//  1               0         0                          4                  MEA  
//  1               0         1                          5                  APAC   
//  1               1         0                          7                  LUODA PA NO fingerprint    
//  1               1         1                          7                  LUODA PA    
/***********************************************/
unsigned char get_image_id(unsigned char board_id)
{
    unsigned char image_id = 0;
    switch(board_id)
    {
    case 0:
    case 1:
...
//[BUGFIX]-Add-BEGIN by SCDTABLET.lifang.cheng,12/12/2016,3510383,
//add feature multi-MD
#if defined(CONFIG_PROJECT_PIXI5104G) && defined(TARGET_BUILD_MMITEST)
        unsigned char board_id = 0;
        unsigned char gpio1_state = 0;
        unsigned char gpio2_state = 0;

        mt_set_gpio_mode((90 | 0x80000000), GPIO_MODE_00);
        mt_set_gpio_dir((90 | 0x80000000), GPIO_DIR_IN);
        mt_set_gpio_pull_enable((90 | 0x80000000), GPIO_PULL_DISABLE);

        mt_set_gpio_mode((122 | 0x80000000), GPIO_MODE_00);
        mt_set_gpio_dir((122 | 0x80000000), GPIO_DIR_IN);
        mt_set_gpio_pull_enable((122 | 0x80000000), GPIO_PULL_DISABLE);

        gpio1_state = mt_get_gpio_in(122 | 0x80000000);
        gpio2_state = mt_get_gpio_in(90 | 0x80000000);
 //[BUGFIX]-Add-BEGIN by SCDTABLET.lifang.cheng,02/14/2017,4130726,
 //optimize codes
        board_id =  ((gpio2_state & 0x01)<< 0x01)|(gpio1_state & 0x01);
...
//get board id from command line
        ...
        unsigned char board_id = 0;
        unsigned char image_id = 0;
        get_board_id(md_id,&board_id);
	image_id = get_image_id(board_id);
#endif
//[BUGFIX]-Add-END by SCDTABLET.lifang.cheng
...
#if defined(CONFIG_PROJECT_PIXI5104G) && defined(TARGET_BUILD_MMITEST)
        snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_%d", type_str[feature_val],board_id);
//[BUGFIX]-Add-BEGIN by SCDTABLET.lifang.cheng,01/11/2017,3585289,
//get board id from command line
#elif defined(CONFIG_MULTI_MD)
         snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_%d", type_str[feature_val],image_id);
#else
