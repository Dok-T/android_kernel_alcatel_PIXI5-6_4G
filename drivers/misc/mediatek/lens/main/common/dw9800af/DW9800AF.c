...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    ...
    unsigned short  i4RetValue = 0;                                                                                                                                                 
    char pBuff[2];
    char puSendCmd1[2] =  { 0x03 };
    char puSendCmd2[2] =  { 0x04 };
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd1, 1);
    i2c_master_recv(g_pstAF_I2Cclient, pBuff , 1);
    i4RetValue = (u16)pBuff[1] << 8 ;
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd2, 1);
    i2c_master_recv(g_pstAF_I2Cclient, pBuff , 1);
    i4RetValue |= (u16)pBuff[1] ;
     
    printk("current_pos read from regster %x",i4RetValue);
    *a_pu2Result = i4RetValue ;
    return 0;
}
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    char puSendCmd1[2] =  { 0x03 , (char)(((a_u2Data  >> 8) & 0x03))};
...
static void DW9800AF_reduce_lens_power_down_noise(void)
{

}


static void DW9800AF_Init(void)
{
	//printk("cuckoo %s\n",__func__);
}

static void DW9800AF_DeInit(void)
{
	//printk("cuckoo %s\n",__func__);
	DW9800AF_reduce_lens_power_down_noise();
}
