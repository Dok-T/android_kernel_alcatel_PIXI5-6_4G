...
P_MSDU_INFO_T qmEnqueueTxPackets(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	do {
		...
		if (1) {
            ...
            //[BUGFIX]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/16/2017, task-4014769, add log for mtk[IP_REACHABILITY_LOST]
            DBGLOG(QM, INFO, "Trace ARP Frame Sta_Rec_Index %d\n", prCurrentMsduInfo->ucStaRecIndex);
...
#endif
#if ARP_MONITER_ENABLE
                DBGLOG(QM, INFO, "Trace ARP Frame Src %d\n", prCurrentMsduInfo->eSrc);//[BUGFIX]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/16/2017, task-4014769, add log for mtk[IP_REACHABILITY_LOST]
...
//[FEATURE]-Add BEGIN by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
	DBGLOG(INIT, INFO, "Trace ARP EthType 0x%x\n", u2EtherType);
    DBGLOG(INIT, INFO, "Trace ARP AP_IP %d.%d.%d.%d\n", apIp[0],apIp[1],apIp[2],apIp[3]);
    DBGLOG(INIT, INFO, "Trace ARP DestIP %d.%d.%d.%d\n", pucData[ETH_TYPE_LEN_OFFSET + 26],
    pucData[ETH_TYPE_LEN_OFFSET + 27],
    pucData[ETH_TYPE_LEN_OFFSET + 28],
    pucData[ETH_TYPE_LEN_OFFSET + 29]);

//[FEATURE]-Add END by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
    //[BUGFIX]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/16/2017, task-4014769, add log for mtk[IP_REACHABILITY_LOST]
    DBGLOG(INIT, INFO, "Trace ARP EthType 0x%x\n", u2EtherType);
	...
	//[FEATURE]-Add BEGIN by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
	DBGLOG(INIT, INFO, "Trace ARP Opcode %d\n", arpOpCode);
  DBGLOG(INIT, INFO, "Trace ARP arpMoniterCnt %d\n", arpMoniter);
//[FEATURE]-Add END by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
    DBGLOG(INIT, INFO, "Trace ARP Opcode %d\n", arpOpCode);//[BUGFIX]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/16/2017, task-4014769, add log for mtk[IP_REACHABILITY_LOST]
...
VOID qmHandleRxArpPackets(P_ADAPTER_T prAdapter, P_SW_RFB_T prSwRfb)
{
	...
	if (!pucData)
		return;
	u2EtherType = (pucData[ETH_TYPE_LEN_OFFSET] << 8) | (pucData[ETH_TYPE_LEN_OFFSET + 1]);
//[FEATURE]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
	 DBGLOG(INIT, INFO, "Trace ARP RX EthType 0x%x\n", u2EtherType);
	...
	DBGLOG(INIT, INFO, "Trace ARP RX Opcode 0x%x\n", arpOpCode);//[FEATURE]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
	if (arpOpCode == ARP_PRO_RSP) {
		arpMoniter = 0;
		if (prBssInfo && prBssInfo->prStaRecOfAP && prBssInfo->prStaRecOfAP->aucMacAddr) {
//[FEATURE]-Add BEGIN by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
		DBGLOG(INIT, INFO, "Trace ARP RX AP_Mac %pM\n", prBssInfo->prStaRecOfAP->aucMacAddr);
    DBGLOG(INIT, INFO, "Trace ARP RX Pkt_Mac %pM\n", &(pucData[ETH_TYPE_LEN_OFFSET + 10]));
//[FEATURE]-Add END by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
			if (EQUAL_MAC_ADDR(&(pucData[ETH_TYPE_LEN_OFFSET + 10]), /* source hardware address */
				...
				DBGLOG(INIT, INFO, "get arp response from AP %d.%d.%d.%d\n",//[FEATURE]-Mod by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
					apIp[0], apIp[1], apIp[2], apIp[3]);
			}
		}
	}
...
{
	DBGLOG(INIT, INFO, "Trace ARP Enter Reset\n");//[FEATURE]-Add by SCDTABLET.(binghua.fang@tcl.com), 01/24/2017, task-4014769, add log for mtk[ALPS03118941][IP_REACHABILITY_LOST]
	arpMoniter = 0;
	kalMemZero(apIp, sizeof(apIp));
	kalMemZero(gatewayIp, sizeof(gatewayIp));
}
