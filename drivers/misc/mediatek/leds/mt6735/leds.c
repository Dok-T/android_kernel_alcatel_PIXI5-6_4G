...
#include "ddp_pwm.h"
#include "mtkfb.h"
//#define  GPIO_CHARGE_LED_PIN	(GPIO96 | 0x80000000)
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	unsigned int BacklightLevelSupport =
	    Cust_GetBacklightLevelSupport_byPWM();
//	LEDS_DEBUG("brightness_set_cust:The mode=%d level=%d!!!!!\n",cust->mode,level);
	switch (cust->mode) {
...
{
//	static bool test_time = true;
//	int ret = 0;
	struct mt65xx_led_data *led_data =
	    container_of(led_cdev, struct mt65xx_led_data, cdev);
	/* unsigned long flags; */
	/* spin_lock_irqsave(&leds_lock, flags); */
//[BUGFIX]-ADD-BEGIN by bao.li@tcl.com,2016/11/23,alm 3461768
#if 0
	if (test_time == true && (strcmp(led_data->cust.name, "red") == 0)) {

			ret = mt_set_gpio_mode(GPIO_CHARGE_LED_PIN, GPIO_MODE_00);
			if (0 != ret)
				battery_log(BAT_LOG_CRTI,"GPIO_CHARGE_LED_PIN mt_set_gpio_mode fail\n");

			ret = mt_set_gpio_dir(GPIO_CHARGE_LED_PIN, GPIO_DIR_OUT);
			if (0 != ret)
				battery_log(BAT_LOG_CRTI,"GPIO_CHARGE_LED_PIN mt_set_gpio_dir fail\n");

			ret = mt_set_gpio_out(GPIO_CHARGE_LED_PIN,GPIO_OUT_ONE);
				battery_log(BAT_LOG_CRTI,"GPIO_CHARGE_LED_PIN mt_get_gpio_in =%d\n",ret);
			ret = mt_get_gpio_out(GPIO_CHARGE_LED_PIN);

			printk("GPIO_CHARGE_LED_PIN the gpio state=%d\n\n",ret);

			test_time = false;
		}
#endif
//[BUGFIX]-ADD-END by bao.li@tcl.com
#ifdef CONFIG_MTK_AAL_SUPPORT
...
//	printk("GPIO_CHARGE_LED_PIN the cust.name =%s data->level %d level %d\n\n",led_data->cust.name,led_data->level,level);
	if (led_data->level != level) {
		led_data->level = level;
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			LEDS_DEBUG("Set NLED directly %d at time %lu\n",
