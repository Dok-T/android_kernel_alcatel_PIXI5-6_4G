...
//#define SUPPORT_COMPATIABLE_MODE //dws only have one device node for gsensor
...
static int bma_set_powermode(struct i2c_client *client, enum BMA_POWERMODE_ENUM power_mode);
...
#ifdef SUPPORT_COMPATIABLE_MODE
    {.compatible = "mediatek,bma2xx"},
#else
...
static int bma_read_raw_data(struct i2c_client *client, s16 data[BMA_AXES_NUM])
{
    ...
    if (priv->power_mode == BMA_SUSPEND_MODE) {
        ...
        if (err < 0) {
            ...
            GSE_ERR("set power mode to normal!\n");
            msleep(20);
        }
    ...
    if (priv->sensor_type == BMA220_TYPE) {/* BMA220 */
        ...
        err = bma_i2c_read_block(client, BMA250_REG_DATAXLOW, buf_tmp, 6);
        ...
        u8 buf_tmp[BMA_DATA_LEN] = {0};
        err = bma_i2c_read_block(client, BMA2X2_REG_DATAXLOW, buf_tmp, 6);
...
static int bma_init_client(struct i2c_client *client, int reset_cali)
{
    ...
    err = bma_set_datarate(client, BMA_DATARATE_100HZ);
    ...
    /*recover the gsensor status*/
    err = bma_set_powermode(client, enable_status); //BMA_SUSPEND_MODE
                                      ...
                                      const char *buf, size_t count)
{
    ...
    if (err == 0) {
        err = bma_set_powermode(obj->client, (enum BMA_POWERMODE_ENUM)(!!(power_mode)));
...
static int gsensor_enable_nodata(int en)
{
    ...
    if (((en == 0) && (sensor_power == false)) || ((en == 1) && (sensor_power == true))) {
        ...
        if (atomic_read(&obj_i2c_data->suspend) == 0) {
            err = bma_set_powermode(obj_i2c_data->client, enable_status);
...
static int gsensor_set_delay(u64 ns)
{
    ...
    enum BMA_DATARATE_ENUM sample_delay;
    ...
    err = bma_set_datarate(obj_i2c_data->client, sample_delay);
                         ...
                         const struct i2c_device_id *id)
{
    ...
    for (retry = 0; retry < 3; retry++) {
        err = bma_init_client(new_client, 1);

        if (0 != err) {
            GSE_ERR("bma_device init cilent fail time: %d\n", retry);
            continue;
    ...
    err = bma_create_attr(&bma_init_info.platform_diver_addr->driver);
    ...
    if (err) {
        ...
        goto exit_register_support_info;
    }
    ...
    GSE_LOG("%s: OK\n", __func__);
    return 0;
exit_register_support_info:
exit_register_data_path:
exit_register_control_path:
    bma_delete_attr(&bma_init_info.platform_diver_addr->driver);
...
static int bma_i2c_remove(struct i2c_client *client)
{
    ...
    err = bma_delete_attr(&bma_init_info.platform_diver_addr->driver);
...
{
    GSE_FUN();
    GSE_LOG("Bosch accelerometer driver version: %s\n", BMA_DRIVER_VERSION);
