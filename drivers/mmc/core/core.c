...
extern void sd_power_control(u32 on);
...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	...
	printk("***** mmc_rescan_try_freq start, host->index=%d *****\n",host->index);
#ifdef CONFIG_MMC_DEBUG
	...
	if (!mmc_attach_mmc(host))
		return 0;
	/*add by sansan.gan to fix lost sd card on pixi56 begin,pr 3718871,3649853*/
	if(host->index == 1)
	{
		printk("*****sd re-attach start *****\n");
		sd_power_control(0);
		msleep(50);
		sd_power_control(1);
		msleep(50);
		...
		if (!mmc_attach_sd(host))
			return 0;
		printk("***** sd re-attach failed *****\n");
	}
	/*add by sansan.gan to fix lost sd card on pixi56 end,pr 3718871,3649853*/	
