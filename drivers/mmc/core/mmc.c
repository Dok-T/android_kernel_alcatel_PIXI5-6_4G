...
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com), for fw_rev 09/20/2016*/
        card->ext_csd.zgxfw = ext_csd[EXT_CSD_FIRMWARE_VERSION];
/*[BUGFIX]-Mod-END by TCTSZ.(wenlong.huang@tcl.com),*/
...
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com), for fw_rev 08/17/2016*/
	...
	if (!oldcard) {
		...
		if (err)
			goto free_card;
                ...
                {
                        pr_info("emcp_samsung\n");
                }
                else if(card->cid.manfid == CID_MANFID_TOSHIBA)
                {
                        pr_info("emcp_toshiba\n");
                }
                else if((card->cid.manfid == CID_MANFID_SANDISK) || (card->cid.manfid == CID_MANFID_SANDISK_NEW))
                {
                        pr_info("emcp_sandisk\n");
                }
