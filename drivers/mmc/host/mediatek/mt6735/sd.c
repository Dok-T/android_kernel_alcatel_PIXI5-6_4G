...
#endif
void sd_power_control(u32 on)
{
	printk("######%s######\n",__func__);
	msdc_ldo_power(on, POWER_LDO_VMCH, VOL_3000, &g_msdc1_flash);
	
}
EXPORT_SYMBOL(sd_power_control);
...
static void msdc_apply_ett_settings(struct msdc_host *host, int mode)
{
	...
	sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, 0);
	mdelay(100);  //zhongzhu add for crc tune failed,2017-4-26
}
...
/* Add by [wei.zhang] at [2016.10.09] for device info begin */
extern struct device* get_deviceinfo_dev(void);
static u8 *emmc_spt_tab[][4] = {
    /*:::*/
    {"150100464E58324D42", "KMFNX0012M_B214",		"Samsung",		"8G+1G"},
    {"150100464531324D42", "KMFE10012M_B214",		"Samsung",	"16G+1G"},
    {"150100514531334D42", "KMQE10013M_B318",		"Samsung",	"16G+2G"},
    {"150100524531424D42", "KMRE1000BM_B512",		"Samsung",	"16G+3G"},
    {"13014E51324A393652", "MT29TZZZ5D6JKFRL_107W_96R",		"Micron",		"16G+2G"},
    {"13014E51334A393652", "MT29TZZZ5D6EKFRL_107W_96R",		"Micron",	"16G+2G"},
    {"13014E51324A393551", "MT29TZZZ8D5JKEZB",		"Micron",	"8G+1G"},
    {"110100303136473330", "TYE0HH221668RA",		"toshiba",  "16G+2G" },
    {"110100303136473330", "THGBMHG7C1LBAIL",		"Toshiba",  "16G"},
    {"450100444634303136", "SDADF4AP_16G",		            "Sandisk",  "16G+2G"},
    {"90014A484247346132", "H9TQ26ADFTBCUR_KUM",		    "Hynix",  "32G+3G"},
    {"700100454841434541", "16EMCP16_EL3DT527",		        "Kingston",  "16G+2G"},
...
/* Add by [wei.zhang] at [2016.10.09] for device info end */
...
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(gaoxiang.zou@tcl.com)  12/29/2016*/
/* Add by wei.zhang [2016.10.09] for device_info begin */
	if (host->hw->host_function == MSDC_EMMC) {
		create_memory_id_node();
	}
/* Add by wei.zhang [2016.10.09] for device_info end */
    /*[BUGFIX]-Mod-END by TCTSZ.(gaoxiang.zou@tcl.com),*/
