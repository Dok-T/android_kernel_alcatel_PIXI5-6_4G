...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.11.29, fix tp's power source*/
extern void tpd_power_2v8_control(int enable);
/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2016.11.29*/

//#define MT_PROTOCOL_B
//#define TPD_PROXIMITY
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com), enable TP GESTRUE and AUTO_UPGRADE function 06/09/2016*/
#define FTS_GESTRUE
#define TPD_AUTO_UPGRADE				// if need upgrade CTP FW when POWER ON,pls enable this MACRO
/*[BUGFIX]-Mod-END by TCTSZ.(wenlong.huang@tcl.com), enable TP GESTRUE and AUTO_UPGRADE function*/
...
#endif
//[BUGFIX]-MOL-BEGIN by SCDTABLET.(wenlong.huang@tcl.com),06/20/2016
//extern int open_gesture;
extern int gesture_wakeup_flag;
static int gesture_wakeup_enable = 1;
extern unsigned int gtp_fw_version;
extern u8 gtp_sensor_id;
//[BUGFIX]-MOL-END by SCDTABLET.(wenlong.huang@tcl.com),06/20/2016,
#ifdef FLIP_COVER_WINDOW
...
static void check_gesture(int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_DOUBLECLICK:
				/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/
				input_report_key(tpd->dev, KEY_WAKEUP, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_WAKEUP, 0);
				input_sync(tpd->dev);
				/*[BUGFIX]-Mod-END by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/
			break;
		case GESTURE_O:
	 ...
	 do
	 {
		// mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
		 ...
		 //printk("tpd touch_event_handler\n");
	 	 #ifdef FTS_GESTRUE
		 if ((1 == gesture_wakeup_enable) && (1 == gesture_wakeup_flag))
		 {
...
#ifdef FLIP_COVER_WINDOW
int flip_cover_window(int open_window)
{
	int ret=0;
	ret = fts_write_reg(i2c_client,0xC1,open_window);	
	if(ret < 0)
	{
		printk("Fts write err: %s %d.\n",__func__,open_window);
		return ret;
	}
	return 0;
}
...
{	 
	...
	nRetVal = regulator_set_voltage(tpd->reg, 2800000, 2800000); // power on Touch IC.
	if (nRetVal)
	{
	...
	input_set_capability(tpd->dev, EV_KEY, KEYCODE_DOUBLECLICK);
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/
...
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/		
	...
	__set_bit(KEYCODE_DOUBLECLICK, tpd->dev->keybit);
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/
	__set_bit(KEY_WAKEUP, tpd->dev->keybit);
/*[BUGFIX]-Mod-BEGIN by TCTSZ.(wenlong.huang@tcl.com) DOUBLECLICK wakup 06/30/2016*/
#endif
...
static int tpd_local_init(void)
{
    ...
    #if 0
	tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");       //removed by lizhi.wu@tcl.com 12-1 for no need to do these
    #endif

    if (tpd_load_status == 1) {
        printk(" add touch panel driver already.\n");
        return -1;
    }
	...
	tpd_type_cap = 1;
//	sprintf(ctp_module_name,"FT5436:TRULY:SUP1:0x12");
	return 0; 
}
...
//[BUGFIX]-MOL-END by SCDTABLET.(wenlong.huang@tcl.com),06/20/2016,
	{
		...
		if (fts_updateinfo_curr.CHIP_ID==0x54)
		{
			printk("hello enable gesture mode\n");
		...
		//[BUGFIX]-MOL-BEGIN by SCDTABLET.(zhongzhu.peng@tcl.com),07/19/2016
		//points exist on the panel when resume system.
			input_report_key(tpd->dev, BTN_TOUCH, 0);	
			input_mt_sync(tpd->dev);
			input_sync(tpd->dev);
		//[BUGFIX]-MOL-END by SCDTABLET.(zhongzhu.peng@tcl.com),07/19/2016	
		return;
	}
#endif
 	tpd_halt = 1;
