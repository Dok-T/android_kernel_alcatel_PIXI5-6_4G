...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH + 4] + (buf[GTP_ADDR_LENGTH + 5]
                               << 8);
    return SUCCESS;
}
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
    ...
    if (HEADER_FW_READY == got_file_flag) {
        if (SEEK_SET == set_or_end)
            ...
            memcpy(buf, &gtp_default_FW[update_msg.fw_total_len + FW_HEAD_LENGTH - offset],
                   length);
                                   ...
                                   struct st_fw_head *fw_head, char *path)
{
    ...
    if (NULL != path) {
        ...
        GTP_INFO("FILE HARDWARE INFO:%02x%02x%02x%02x",
                 fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2],
                 fw_head->hw_info[3]);
        GTP_INFO("FILE PID:%s", fw_head->pid);
    ...
    for (i = FW_HEAD_LENGTH; i < update_msg.fw_total_len; i += 2)
        fw_checksum += (gtp_hotknot_calibration_section0[i] << 8) +
