                           ...
                           int len)
{
    ...
    GTP_DEBUG("i2c_read_bytes to device %02X address %04X len %d", client->addr,
              addr, len);
                            ...
                            int len)
{
    ...
    GTP_DEBUG("i2c_write_bytes to device %02X address %04X len %d", client->addr,
              addr, len);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    if ((!cfg_info_len[1]) && (!cfg_info_len[2]) &&
        ...
        (!cfg_info_len[5]) && (!cfg_info_len[0])) {
        GTP_ERROR("All config group length was zero\n");
        pnl_init_error = 1;
        return -1;
    } else {
#ifdef CONFIG_GTP_COMPATIBLE_MODE
...
static void gtp_esd_check_func(struct work_struct *work)
{
...
#if FLASHLESS_FLASH_WORKROUND
    {
        ...
        while (retry++ < 3) {
            ret = i2c_read_bytes_non_dma(i2c_client_point, 0x8140, versionBuff,
                                         sizeof(versionBuff));
...
#else
        if (tpd_halt || (gtp_loading_fw == 1)) {
            GTP_INFO("return for interrupt after suspend...	");
#endif
            goto exit_work_func;
        }
...
static void tpd_suspend(struct device *h)
{
    ...
    if (ret < 0)
        GTP_ERROR("GTP enter %s failed.",
                (gesture_wakeup_flag ? "doze" : "suspend"));
#else
    disable_irq(touch_irq);
    ret = gtp_enter_sleep(i2c_client_point);
