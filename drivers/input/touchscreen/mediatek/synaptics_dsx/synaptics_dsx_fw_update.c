...
#define FW_PATH_EXTERNAL "/sdcard/synaptics_fw_update.img"
...
static ssize_t synaptics_sys_fw_update(struct device *dev,
...
static struct device_attribute attrs[] = {
	...
	__ATTR(fw_update, 0660,
			synaptics_sys_fw_update,
			NULL),
...
static int update_fw_from_sdcard(struct synaptics_rmi4_data *rmi4_data)
{
	struct file *fp;
    mm_segment_t old_fs;
    ...
    int ret = fw_err_none;
	mutex_init(&lock);
	dev_err(&rmi4_data->i2c_client->dev, "%s [START]\n", __func__);
    ...
    old_fs = get_fs();
    set_fs(KERNEL_DS);
	fp = filp_open(FW_PATH_EXTERNAL, O_RDONLY, S_IRUSR);

	if (IS_ERR(fp)) {
			dev_err(&rmi4_data->i2c_client->dev, "%s [ERROR] file_open - path[%s]\n",
					__func__, FW_PATH_EXTERNAL);
	...
	if (0 < fw_size) {
        ...
        dev_err(&rmi4_data->i2c_client->dev,"%s - path[%s] size[%ld]\n", __func__, FW_PATH_EXTERNAL, (long int)fw_size);

        if (nread != fw_size) {
            dev_err(&rmi4_data->i2c_client->dev, "%s [ERROR] vfs_read - size[%ld] read[%ld]\n",
                    __func__, (long int)fw_size, (long int)nread);
            ret = fw_err_file_read;

        } else {
            //Update firmware
            ret = synaptics_fw_updater(fw_data);
        }

        kfree(fw_data);

    } else {
        dev_err(&rmi4_data->i2c_client->dev, "%s [ERROR] fw_size [%ld]\n",__func__, (long int)fw_size);
        ret = fw_err_file_read;
    }
...
ERROR:
    ...
    if(ret < fw_err_none)
        dev_err(&rmi4_data->i2c_client->dev, "%s [ERROR]\n", __func__);
    else
        dev_err(&rmi4_data->i2c_client->dev, "%s [DONE]\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_err(&rmi4_data->i2c_client->dev, "%s [START]\n", __func__);
	retval = update_fw_from_sdcard(rmi4_data);
	switch(retval) {
		case fw_err_none:
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (get_boot_mode() != META_BOOT) {
	    ...
	    dev_info(&rmi4_data->i2c_client->dev,
	            "Ignore firmware update in meta mode!");
	}
#endif
