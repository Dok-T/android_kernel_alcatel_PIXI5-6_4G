...
#define CHARGER_DETECTED_BIT        5
static atomic_t charge_bit_enable = ATOMIC_INIT(0);
static struct synaptics_rmi4_data *g_rmi4_data = NULL;
static void synaptics_rmi4_update_charge_bit(void);
void synaptics_rmi4_set_charge_bit(bool set);
EXPORT_SYMBOL(synaptics_rmi4_set_charge_bit);
#endif
...
static void synaptics_rmi4_update_charge_bit(void)
{
     int retval = -1, charge_enable_status = atomic_read(&charge_bit_enable);
     unsigned char charge_ctrl = 0, charge_bit = 0, feed_back;

     if (g_rmi4_data == NULL)
         return;

     retval = synaptics_rmi4_i2c_read(g_rmi4_data,
             g_rmi4_data->f01_ctrl_base_addr,
             &charge_ctrl,
             sizeof(charge_ctrl));
     if (retval < 0) {
         pr_err("Failed to read charge bit control register!\n");
         return;
     }

     charge_bit = ((charge_ctrl >> 5) & 0x1);
     pr_info("%s:charge_ctrl = 0x%x, charge bit = 0x%x\n",
             __func__, charge_ctrl, charge_bit);
     if (((charge_enable_status == 1) && charge_bit) ||
             ((charge_enable_status == 0) && !charge_bit)) {
         pr_info("%s:Charge bit was already updated!\n", __func__);
         return;
     }

     if (charge_enable_status)
         charge_ctrl = (charge_ctrl | (1 << CHARGER_DETECTED_BIT));
     else
         charge_ctrl = (charge_ctrl & (~(1 << CHARGER_DETECTED_BIT)));

     pr_info("%s:Write 0x%x to f01 control base address!\n",
     ...
     retval = synaptics_rmi4_i2c_write(g_rmi4_data,
             ...
             sizeof(charge_ctrl));
     if (retval < 0) {
         pr_err("%s:Failed to write f01 control register!\n", __func__);
         return;
     }
     ...
     retval = synaptics_rmi4_i2c_read(g_rmi4_data,
             ...
             sizeof(feed_back));
     if (retval < 0) {
         pr_err("%s:Failed to read feed back value!\n", __func__);
         return;
     }

     if (feed_back != charge_ctrl) {
         pr_err("%s:Write value is 0x%x, but read value is 0x%x!\n",
                 __func__, charge_ctrl, feed_back);
     } else {
         pr_err("%s:Success to update charge bit!\n", __func__);
     }
}
...
void synaptics_rmi4_set_charge_bit(bool set)
{
    ...
    pr_err("%s:%s charge bit!\n", __func__, (set ? "Set" : "Clear"));

    if (g_rmi4_data == NULL) {
        pr_err("Global data was't initialized!\n");
        atomic_set(&charge_bit_enable, val);
        return;
    }
    ...
    if (g_rmi4_data->sensor_sleep == false) {
        ...
        synaptics_rmi4_update_charge_bit();
    } else {
        atomic_set(&charge_bit_enable, val);
    }
}
...
//	tpd_gpio_as_int(1);// eint 1
	...
	if (atomic_read(&charge_bit_enable))
	    synaptics_rmi4_update_charge_bit();
#endif
...
static int tpd_local_init(void)
{
	if (tpd_load_status == 1) {
        TPD_DMESG("A suitable touch driver was already found!\n");
        return -1;
    }
	...
	if (tpd_load_status == 0) {
        ...
        TPD_DMESG("Failed to add synaptics touch driver!\n");
        return -ENODEV;
    }
#ifdef TPD_HAVE_BUTTON
