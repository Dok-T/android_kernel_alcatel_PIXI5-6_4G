...
static ssize_t mip_dev_fs_read(struct file *fp, char *rbuf, size_t cnt,
                               loff_t *fpos)
{
...
static ssize_t mip_dev_fs_write(struct file *fp, const char *wbuf, size_t cnt,
                                loff_t *fpos)
{
    struct mip_ts_info *info = fp->private_data;
    u8 *buf;
    ...
    if(cmd == 1) {
        dev_dbg(&info->client->dev, "%s - cmd[%d] w_len[%u] r_len[%d]\n",
        ...
        print_hex_dump(KERN_ERR, MIP_DEVICE_NAME" : input ", DUMP_PREFIX_OFFSET,
                16, 1, wbuf, cnt, false);
        print_hex_dump(KERN_ERR, MIP_DEVICE_NAME" : output ", DUMP_PREFIX_OFFSET, 
                16, 1, info->dev_fs_buf, buf[cnt - 2], false);
#endif
    } else if(cmd == 2) {
        MIP_DEBUG("%s - cmd[%d] w_len[%d]\n", __func__, cmd, (cnt - 1));
...
static int mip_proc_table_data(struct mip_ts_info *info, u8 size,
                               u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 row_num,
...
int mip_run_test(struct mip_ts_info *info, u8 test_type)
{
    ...
    //print data
    if(mip_proc_table_data(info, size, data_type_size, data_type_sign, buf_addr_h,
...
static ssize_t mip_sys_fw_update_section(struct device *dev,
...
static ssize_t mip_sys_fw_path_store(struct device *dev,
...
static ssize_t mip_sys_fw_path_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    u8 data[255];
...
static ssize_t mip_sys_info(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    u8 data[255];
...
static ssize_t mip_sys_power_on(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
...
static ssize_t mip_sys_power_off(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
...
static ssize_t mip_sys_reboot(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
...
static ssize_t mip_sys_mode_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    u8 wbuf[8];
...
static ssize_t mip_sys_mode_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    u8 data[255];
...
static ssize_t mip_sys_image(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    int ret;
...
static ssize_t mip_sys_test(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    int ret;
...
static ssize_t mip_sys_test_short(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    int ret;
...
static ssize_t mip_sys_wakeup_gesture(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct mip_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
...
static DEVICE_ATTR(fw_path, 0644, mip_sys_fw_path_show, mip_sys_fw_path_store);
...
#endif
static DEVICE_ATTR(mode_glove, 0644, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_charger, 0644, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_cover_window, 0644, mip_sys_mode_show,
        mip_sys_mode_store);
static DEVICE_ATTR(mode_palm_rejection, 0644, mip_sys_mode_show,
        mip_sys_mode_store);
static DEVICE_ATTR(mode_edge, 0644, mip_sys_mode_show, mip_sys_mode_store);
...
int mip_sysfs_create(struct mip_ts_info *info)
{
    ...
    info->image_buf = kzalloc(sizeof(int) * ((info->node_x + info->node_y +
                              info->node_key + 1) * (info->node_x + info->node_y + info->node_key + 1)),
                              GFP_KERNEL);
    MIP_DEBUG("%s [DONE]\n", __func__);
    return 0;
}
