...
struct pinctrl_state *power_ldo_disable, *power_ldo_enable;
#endif
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.11.29, fix tp's power source*/
#ifdef CONFIG_PROJECT_PIXI5104G 
void tpd_power_2v8_control(int enable)
{
    mutex_lock(&tpd_set_gpio_mutex);
    if (enable == 0)        //power_off
    {
        printk("%s: power off!\n", __func__);
        pinctrl_select_state(pinctrl1, power_ldo_disable);
    }
    else
    {
        printk("%s: power on!\n", __func__);
        pinctrl_select_state(pinctrl1, power_ldo_enable);
    }
    mutex_unlock(&tpd_set_gpio_mutex);
}
#endif
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.11.29, fix tp's power source*/
    #ifdef CONFIG_PROJECT_PIXI5104G
    power_ldo_disable = pinctrl_lookup_state(pinctrl1, "power_ldo_disable");
    if (IS_ERR(power_ldo_disable)) {
        ret = PTR_ERR(power_ldo_disable);
        dev_err(&pdev->dev, "fwq Cannot find touch pinctrl power_ldo_disable!\n");
        return ret;
    }
    power_ldo_enable = pinctrl_lookup_state(pinctrl1, "power_ldo_enable");
    if (IS_ERR(power_ldo_enable)) {
        ret = PTR_ERR(power_ldo_enable);
        dev_err(&pdev->dev, "fwq Cannot find touch pinctrl power_ldo_enable!");
        return ret;
    }
    #endif
	/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2016.11.29*/
