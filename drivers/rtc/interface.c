...
{
	struct rtc_time tm;
	ktime_t now, scheduled;
	int err;

	err = rtc_valid_tm(&alarm->time);
	if (err)
		return err;
	scheduled = rtc_tm_to_ktime(alarm->time);
	...
	if (scheduled.tv64 <= now.tv64) {
		pr_emerg("alarm: %d/%d/%d, %d:%d:%d (%lld)\n", alarm->time.tm_year, alarm->time.tm_mon,
			alarm->time.tm_mday, alarm->time.tm_hour, alarm->time.tm_min, alarm->time.tm_sec, scheduled.tv64);
		pr_emerg("now: %d/%d/%d, %d:%d:%d (%lld)\n", tm.tm_year, tm.tm_mon, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec, now.tv64);
		pr_emerg("%s, -ETIME\n", __func__);
		return -ETIME;
	}
	/*
