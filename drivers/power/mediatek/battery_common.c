...
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_current
#ifdef CONFIG_TCT_SET_BATTERY_CURRENT
int g_charging_set_current = -1;
EXPORT_SYMBOL(g_charging_set_current);
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016
...
/*[FEATURE]-Add-BEGIN by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_CHARGE_SWITCH
extern void synaptics_rmi4_set_charge_bit(bool set);
#endif
/*[FEATURE]-Add-END by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
#if !defined(CONFIG_POWER_EXT)
...
struct battery_data {
	...
	int present_smb;
	int adjust_power;
//[BUGFIX]-Mod-BEGIN by SCDTABLET.bao.li@tcl.com,09/12/2016,PR2788790
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_type	
#ifdef TCT_CHECK_BATTERY_TYPE
	int BAT_BATTERY_TYPE;
#endif
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,09/12/2016,PR2788790
};
...
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016,PR2788790
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_current
#ifdef CONFIG_TCT_SET_BATTERY_CURRENT
	POWER_SUPPLY_PROP_BATTERY_CURRENT,
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016
//[BUGFIX]-Mod-BEGIN by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
	POWER_SUPPLY_PROP_TEMP,
//[BUGFIX]-Mod-END by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
};
...
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016,PR2788790
#ifdef CONFIG_TCT_SET_BATTERY_CURRENT
	case POWER_SUPPLY_PROP_BATTERY_CURRENT:
		val->intval = g_charging_set_current;
		break;
#endif
//[BUGFIX]-Mod-BEGIN by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = BMT_status.temperature*10;//0; //data->temp;
		break;
//[BUGFIX]-Mod-END by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (psp == POWER_SUPPLY_PROP_BATTERY_CURRENT) {
		g_charging_set_current = val->intval;
		return ret;
	}
...
static int battery_set_property_is_writeable(struct power_supply *psy,
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (psp == POWER_SUPPLY_PROP_TEMP) {
		printk("[TCL][mark] enable = %d\n",val->intval);
		if(val->intval == 1)
		{
			battery_cmd_thermal_test_mode = 1;
			battery_cmd_thermal_test_mode_value = 25;
		}else{
	...
	else if(psp == POWER_SUPPLY_PROP_BATTERY_CURRENT)
	{
                g_charging_set_current = val->intval;
                return ret;
...
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_current
//#ifdef CONFIG_TCT_SET_BATTERY_CURRENT    //[BUGFIX]-Mod by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
		.set_property = battery_set_property,
		.property_is_writeable = battery_set_property_is_writeable,
//#endif   //[BUGFIX]-Mod by SCDTABLET.wenlong.huang@tcl.com,10/28/2016,TASK3109371
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016
		},
...
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_type
#ifdef TCT_CHECK_BATTERY_TYPE
	.BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_JN,
#endif
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,09/12/2016,PR2788790
};
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.bao.li@tcl.com,14/6/2016,PR2339065
//add charge enable node for enable or disable charging, sys/devices/platform/battery/chg_enable
					 ...
					 const char *buf, size_t count)
{
	...
	printk("sys/devices/platform/battery/ntc_debug: battery_temp = %d\n",battery_temp);
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	if ((BMT_status.UI_SOC == 100) && (BMT_status.charger_exist == KAL_TRUE)
		...
		battery_log(BAT_LOG_FULL,"***********The battery capacity is full\n\n\n\n");
		}
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	//add 1 percent soc tracking time
		...
		{
			if( timer_counter >= DISCHARGE_1Percent_TRACKING_TIME){
				BMT_status.UI_SOC--;
				battery_log(BAT_LOG_CRTI, "1Percent check point: change soc 1 to 0\n");
			}
			battery_log(BAT_LOG_CRTI, "1Percent check poin: timer_counter=%d\n", timer_counter);
			timer_counter ++;
		}
		else
		{
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,2017/01/12,TASK4010380
//only use system off voltage condition
#if 0
...
#endif 
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,2017/01/12,TASK4010380
...
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_type	
#ifdef TCT_CHECK_BATTERY_TYPE
	if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_JN) && (BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_JN))
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_JN;
	else if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_BYD) && (BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_BYD))
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_BYD;
	else if((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_SCUD)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_SCUD))
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_SCUD;
	else if((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_VK)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_VK))
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_VK;
	else
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_UNKNOWN;
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016,PR2788790
//	battery_charging_control(CHARGING_CMD_DUMP_REGISTER, NULL);
//	printk(KERN_ERR"battery_update###############charge_state=0x%x\n\n\n\n",charge_state);
...
//Added by bao.li for Task 3719428 on 2016/12/15 begin
#ifdef TARGET_BUILD_MMITEST
...
#endif
//Added by bao.li for Task 3719428 on 2016/12/15 end
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,2017/01/03,PR3841123
//reset meter when recharging
			battery_meter_reset();
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,2017/01/03,PR3841123
...
for (i = 0; i < 3; i++) {
	...
	if (baton_count >= 3) {
		...
		battery_log(BAT_LOG_CRTI, "[MINI]battery doesn't exist,use defualt SOC %d\n",BMT_status.UI_SOC);
	}
#endif
	...
	battery_log(BAT_LOG_FULL, "**********The batte_status value=%d\n\n\n\n\n",bat_data->BAT_STATUS);
	if (adjust_power != -1) {
...
//NTC behavior modify,add warm charging current limit,and add notify 
	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
		battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Over Temperature !! stop charging!!\n\r");
		g_batt_temp_status = TEMP_POS_HIGH;
		return PMU_STATUS_FAIL;
	} else if(BMT_status.temperature > WARM_CHARGE_TEMPERATURE_X_DEGREE){ 
		if (g_batt_temp_status == TEMP_POS_HIGH) {
			if(BMT_status.temperature <= batt_cust_data.max_charge_temperature_minus_x_degree){
				battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature down from %d to %d(%d), allow low current charging!!\n\r",
		...
		if(BMT_status.temperature >= WARM_CHARGE_TEMPERATURE && g_batt_temp_status == TEMP_POS_NORMAL) {
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature up from %d to %d(%d), allow low current charging!!\n\r",
				    WARM_CHARGE_TEMPERATURE, BMT_status.temperature,
				    WARM_CHARGE_TEMPERATURE_X_DEGREE);
			g_batt_temp_status = TEMP_POS_WARM;
			BMT_status.bat_charging_state = CHR_PRE;
		}
	}else {
		battery_log(BAT_LOG_CRTI,"[BATTERY] Battery Temperature normal, allow full current charging!!\n\r");
		g_batt_temp_status = TEMP_POS_NORMAL;
	}
	return PMU_STATUS_OK;
}
...
//	printk(KERN_ERR"#########data=%d batteryBufferfirst=%d sum=%d index=%d",data,batteryBufferFirst,
//		*(signed int *)sum,index);
}
					    ...
					    unsigned char batteryIndex)
{
	...
	*sum += data;
	...
	battery_log(BAT_LOG_FULL, "bufferdata[%d]= (%d) avgdata=%d\n", batteryIndex, bufferdata[batteryIndex],avgdata);
	return avgdata;
}
...
void mt_battery_GetBatteryData(void)
{
	...
	#if BATT_ID_CHECK_SUPPORT
	unsigned int bat_id_vol; //[FR571989][add battery id]
	#endif
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		if (BMT_status.charger_type == STANDARD_HOST)
		ICharging = min(ICharging,500);
//		printk(KERN_ERR"##########the ICharging=%d",ICharging);
	} else {
		ICharging = 0;
		}
	...
	#if BATT_ID_CHECK_SUPPORT
	bat_id_vol = battery_meter_get_battery_id_voltage(); //[FR571989][add battery id]
	#endif
	...
	BMT_status.ZCV = ZCV;
	#if BATT_ID_CHECK_SUPPORT
	BMT_status.id_vol = bat_id_vol ; //[FR571989][add battery id]
	#endif
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.bao.li@tcl.com,for modify car_tune_value to optimization 2017/1/12
	if (BMT_status.charger_exist == KAL_FALSE) {//discharging
		if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_SCUD)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_SCUD)){
			if (BMT_status.temperature < 10)
			    batt_meter_cust_data.car_tune_value = DISCHARGE_LOW_CAR_TUNE_VALUE_SCUD;  
			else
				batt_meter_cust_data.car_tune_value = DISCHARGE_CAR_TUNE_VALUE_SCUD;
		}
		else if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_BYD)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_BYD))
		{
			if (BMT_status.temperature < 10)
				batt_meter_cust_data.car_tune_value = DISCHARGE_LOW_CAR_TUNE_VALUE_BYD;
			else
				batt_meter_cust_data.car_tune_value = DISCHARGE_CAR_TUNE_VALUE_BYD;
		}
		else if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_VK)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_VK))
		{
			if (BMT_status.temperature < 12)
				batt_meter_cust_data.car_tune_value = DISCHARGE_LOW_CAR_TUNE_VALUE_VK;
			else
				batt_meter_cust_data.car_tune_value = DISCHARGE_CAR_TUNE_VALUE_VK;
		}
		else
		{
			batt_meter_cust_data.car_tune_value = DISCHARGE_CAR_TUNE_VALUE;
		}		
	} else {//charging
		if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_SCUD)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_SCUD)&&(BMT_status.temperature > 10))
		{
			batt_meter_cust_data.car_tune_value = CAR_TUNE_VALUE_SCUD;
		}
		else if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_BYD)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_BYD))
		{
			batt_meter_cust_data.car_tune_value = CAR_TUNE_VALUE_BYD;
		}
		else if ((BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_VK)&&(BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_VK))
		{
			if (BMT_status.temperature < 12)
				batt_meter_cust_data.car_tune_value = LOW_CAR_TUNE_VALUE_VK;
			else
				batt_meter_cust_data.car_tune_value = CAR_TUNE_VALUE_VK;
		}
		else
		{
...
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,2017/1/12
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,02/16/2016,PR-4056347
	battery_log(BAT_LOG_CRTI,
	"AvgVbat=(%d,%d),AvgI=(%d,%d),VChr=%d,AvgT=(%d,%d),SOC=(%d,%d),UI_SOC=%d,ZCV=%d,CHR_Type=%d bcct:%d:%d I:%d Ibat:%d,car_tune:%d\n",
...
//		    BMT_status.IBattery / 10);
		    get_bat_charging_current_level(), BMT_status.IBattery / 10, batt_meter_cust_data.car_tune_value);
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (batt_cust_data.mtk_temperature_recharge_support) {
		...
		//[BUGFIX]-Mod-BEGIN by SCDTABLET.wenlong.huang@tcl.com,10/25/2016
		if ((BMT_status.temperature <= MIN_CHARGE_TEMPERATURE)
		//[BUGFIX]-Mod-END by SCDTABLET.wenlong.huang@tcl.com,10/25/2016
...
/*[FEATURE]-Add-BEGIN by wenlong.huang@tcl.com, 2016/12/13 */
		if(BMT_status.charger_type == STANDARD_CHARGER)
			BMT_status.ICharging = batt_cust_data.ac_charger_current/100 - 40;
		else
			BMT_status.ICharging = batt_cust_data.usb_charger_current/100 - 40;
/*[FEATURE]-Add-END by wenlong.huang@tcl.com, 2016/12/13 */

/*[FEATURE]-Add-BEGIN by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_CHARGE_SWITCH
        synaptics_rmi4_set_charge_bit(true);
#endif
/*[FEATURE]-Add-END by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
...
/*[FEATURE]-Add-BEGIN by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_CHARGE_SWITCH
        synaptics_rmi4_set_charge_bit(false);
#endif
/*[FEATURE]-Add-END by falin.luo@tcl.com, for tp charge bit switch 2016/12/14 */
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,2016/12/22,PR3822693
//add mutex unlock ,it will lead to power off faile
			mutex_unlock(&bat_mutex); //release mutex
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,2016/12/22,PR3822693
			orderly_poweroff(true);
		}
	}
#endif
...
void check_battery_exist(void)
{
	...
	unsigned int battery_status;
	unsigned int i;
/*[FEATURE]-Add-BEGIN by bao.li@tcl.com, 2016/9/1,mini sw pr 2827649 */
#ifdef TARGET_BUILD_MMITEST
        return;
#endif
/*[FEATURE]-Add-END by bao.li@tcl.com */
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.bao.li@tcl.com,12/9/2016,PR2788826
//add charge enable node for enable or disable charging, sys/devices/platform/battery/chg_enable
 		ret_device_file = device_create_file(&(dev->dev), &dev_attr_chg_enable);
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,09/27/2016
		#if (defined MTK_NTC_DEBUG_SIMULATE)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_ntc_debug);
		#endif
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,09/27/2016
