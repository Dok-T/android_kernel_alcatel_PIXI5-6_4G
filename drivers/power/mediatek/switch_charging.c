...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	if (current_limit != -1) {
		g_bcct_flag = 1;
//		printk(KERN_ERR"set_bat_charging_current_limit############type=%d\n\n\n",BMT_status.charger_type);
		switch (BMT_status.charger_type) {
		case STANDARD_HOST:
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.bao.li@tcl.com,09/27/20169
//NTC behavior modify,add warm charging current limit,and add notify 
static void set_temp_warm_charging_cuurent(void)
{
#ifdef CONFIG_USB_IF
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
	...
	if (g_batt_temp_status == TEMP_POS_WARM) {
		...
		g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
		battery_log(BAT_LOG_CRTI, "[BATTERY] Temperature set warm charging current : %d\r\n",
			    g_temp_CC_value);
	}
}
...
{
//	printk(KERN_ERR"select_charging_current#########type=%d\n\n\n",BMT_status.charger_type);
...
#else
		set_temp_warm_charging_cuurent();
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/27/2016
	}
...
//add battery type check function,sysfs node /sys/class/power_supply/battery/battery_current
#ifdef CONFIG_TCT_SET_BATTERY_CURRENT
	if (g_charging_set_current >= 0) {
		g_temp_input_CC_value = g_charging_set_current;
		g_temp_CC_value = g_charging_set_current;
	}
#endif
//[BUGFIX]-Mod-END by SCDTABLET.bao.li@tcl.com,09/12/2016
}
...
*/
//printk(KERN_ERR"select_charging_current_bcct#########type=%d g_bcct_value=%d\n\n\n",BMT_status.charger_type,g_bcct_value);
...
//	printk(KERN_ERR"###############mtk_select_ichg_aicr\n\n\n\n");
	/* Set Ichg, AICR */
	if (get_usb_current_unlimited()) {
		...
		g_temp_CC_value = batt_cust_data.ac_charger_current;
//		printk(KERN_ERR"3###############g_temp_input_CC_value=%d g_temp_CC_value=%d\n\n\n\n",g_temp_input_CC_value,g_temp_CC_value);
		battery_log(BAT_LOG_FULL,
			"USB_CURRENT_UNLIMITED, use batt_cust_data.ac_charger_current\n");
	}
#ifndef CONFIG_MTK_SWITCH_INPUT_OUTPUT_CURRENT_SUPPORT
	else if (g_bcct_flag == -100) {
//		printk(KERN_ERR"6###############g_bcct_flag=%d\n\n\n\n",g_bcct_flag);
...
static void mtk_select_cv(void)
{
	...
	if (batt_cust_data.high_battery_voltage_support)
		if(g_batt_temp_status == TEMP_POS_WARM)
			cv_voltage = BATTERY_VOLT_04_200000_V;//for warm temp, set cv 4.2v
		else
			cv_voltage = BATTERY_VOLT_04_400000_V;//for pixi564g,set cv 4.4v
	else
		cv_voltage = BATTERY_VOLT_04_200000_V;
...
//Added by bao.li for Task 3719428 on 2016/12/15 begin
		...
		|| g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT))
	{
		...
		printk("[BATTERY] to hold: soc=%d, avgICharging=%d, g_platform_boot_mode=%d\n", BMT_status.SOC, BMT_status.ICharging, g_platform_boot_mode);
	}
#endif
//Added by kangbao.li on 2016/12/15 end
	return PMU_STATUS_OK;
}
...
//Modified by bao.li for Task 3719428 on 2016/12/15 begin
	...
	if (BMT_status.SOC <= 65
		...
		|| g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)) {
		BMT_status.bat_charging_state = CHR_CC;
		printk("[BATTERY] exit hold: soc=%d, avgICharging=%d, g_platform_boot_mode=%d\n", BMT_status.SOC, BMT_status.ICharging, g_platform_boot_mode);
	}
#endif
//Modified by kangbao.li on 2016/12/15 end
	/*  Disable charger */
	charging_enable = KAL_FALSE;
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
