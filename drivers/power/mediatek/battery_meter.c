...
struct battery_meter_table_custom_data {
	...
	BATTERY_PROFILE_STRUCT battery_profile_t0[79];
	int battery_profile_t1_size;
	BATTERY_PROFILE_STRUCT battery_profile_t1[79];
	int battery_profile_t2_size;
	BATTERY_PROFILE_STRUCT battery_profile_t2[79];
	int battery_profile_t3_size;
	BATTERY_PROFILE_STRUCT battery_profile_t3[79];
	int battery_profile_temperature_size;
	BATTERY_PROFILE_STRUCT battery_profile_temperature[79];

	int r_profile_t0_size;
	R_PROFILE_STRUCT r_profile_t0[79];
	int r_profile_t1_size;
	R_PROFILE_STRUCT r_profile_t1[79];
	int r_profile_t2_size;
	R_PROFILE_STRUCT r_profile_t2[79];
	int r_profile_t3_size;
	R_PROFILE_STRUCT r_profile_t3[79];
	int r_profile_temperature_size;
	R_PROFILE_STRUCT r_profile_temperature[79];
};
struct battery_meter_table_custom_data batt_meter_table_cust_data;
#endif
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20170321,Defect-4056347
//add battery id debug interface
int id_volt = 0;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20170321,Defect-4056347
...
int __batt_meter_init_cust_data_from_cust_header(void)
{
	...
	memcpy(&batt_meter_table_cust_data.r_profile_t0, &r_profile_t0[g_fg_battery_id], sizeof(r_profile_t0[g_fg_battery_id]));

	batt_meter_table_cust_data.r_profile_t1_size =
	    sizeof(r_profile_t1[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.r_profile_t1, &r_profile_t1[g_fg_battery_id], sizeof(r_profile_t1[g_fg_battery_id]));

	batt_meter_table_cust_data.r_profile_t2_size =
	    sizeof(r_profile_t2[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.r_profile_t2, &r_profile_t2[g_fg_battery_id], sizeof(r_profile_t2[g_fg_battery_id]));
	batt_meter_table_cust_data.r_profile_t3_size =
	    sizeof(r_profile_t3[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.r_profile_t3, &r_profile_t3[g_fg_battery_id], sizeof(r_profile_t3[g_fg_battery_id]));

	for(i = 0; i< batt_meter_table_cust_data.battery_profile_t0_size; i ++)
	{
		printk("battery_profile_t0[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.battery_profile_t0[i].percentage, batt_meter_table_cust_data.battery_profile_t0[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.battery_profile_t1_size; i ++)
	{
		printk("battery_profile_t1[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.battery_profile_t1[i].percentage, batt_meter_table_cust_data.battery_profile_t1[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.battery_profile_t2_size; i ++)
	{
		printk("battery_profile_t2[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.battery_profile_t2[i].percentage, batt_meter_table_cust_data.battery_profile_t3[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.battery_profile_t0_size; i ++)
	{
		printk("battery_profile_t3[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.battery_profile_t3[i].percentage, batt_meter_table_cust_data.battery_profile_t3[i].voltage);
	}	
	
	for(i = 0; i< batt_meter_table_cust_data.r_profile_t0_size; i ++)
	{
		printk("battery_r_profile_t0[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.r_profile_t0[i].resistance, batt_meter_table_cust_data.r_profile_t0[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.r_profile_t1_size; i ++)
	{
		printk("battery_r_profile_t1[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.r_profile_t1[i].resistance, batt_meter_table_cust_data.r_profile_t1[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.r_profile_t2_size; i ++)
	{
		printk("battery_r_profile_t2[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.r_profile_t2[i].resistance, batt_meter_table_cust_data.r_profile_t2[i].voltage);
	}
	for(i = 0; i< batt_meter_table_cust_data.r_profile_t3_size; i ++)
	{
		printk("battery_r_profile_t3[%d][%d]:%d,%d\n", g_fg_battery_id, i, batt_meter_table_cust_data.r_profile_t3[i].resistance, batt_meter_table_cust_data.r_profile_t3[i].voltage);
	}
#endif
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Batt_Temperature_Table[g_fg_battery_id][0].TemperatureR) {
		TBatt_Value = -20;
	} else if (Res <= Batt_Temperature_Table[g_fg_battery_id][16].TemperatureR) {
		TBatt_Value = 60;
	} else {
		RES1 = Batt_Temperature_Table[g_fg_battery_id][0].TemperatureR;
		TMP1 = Batt_Temperature_Table[g_fg_battery_id][0].BatteryTemp;

		for (i = 0; i <= 16; i++) {
			if (Res < Batt_Temperature_Table[g_fg_battery_id][i].TemperatureR) {
				RES1 = Batt_Temperature_Table[g_fg_battery_id][i].TemperatureR;
				TMP1 = Batt_Temperature_Table[g_fg_battery_id][i].BatteryTemp;
			} else {
				RES2 = Batt_Temperature_Table[g_fg_battery_id][i].TemperatureR;
				TMP2 = Batt_Temperature_Table[g_fg_battery_id][i].BatteryTemp;
				break;
			}
		}
...
int BattVoltToTemp(int dwVolt)
{
	...
	TRes = TRes_temp;
#endif
//	printk(KERN_ERR"[debug]BattVoltToTemp the R=%d\n\n\n",(int)TRes);
...
/* 1. Get Raw Data */
	...
	battery_log(BAT_LOG_CRTI, "[FGADC] SWOCV : %d,%d,%d,%d,%d,%d\n",
