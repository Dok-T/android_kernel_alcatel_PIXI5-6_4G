...
static inline int get_gid(struct sdcardfs_inode_info *info) {
	...
	if (sb_info->options.gid == AID_SDCARD_RW) {
		...
		//[BUGFIX]-Add-BEGIN by SCDTABLET.(zhensheng.deng@tcl.com),06/03/2017,4269251
		//add gid permission for dual app
		int valclone_id = 0;
		if(0 != valclone_id){
			int gid = -1;
			int current_uid = from_kuid(&init_user_ns, current_uid());
			int current_userid = multiuser_get_user_id(current_uid);

			if(info->userid == 0 && (current_userid == valclone_id)) {
				gid = multiuser_get_uid(current_userid, sb_info->options.gid);
			} else if ((info->userid == ((unsigned int)valclone_id)) && (current_userid == 0)) {
				gid = multiuser_get_app_id(sb_info->options.gid);
			} else {
				gid = multiuser_get_uid(info->userid, sb_info->options.gid);
			}
			return gid;
		}
		//[BUGFIX]-Add-END by SCDTABLET.(zhensheng.deng@tcl.com),06/03/2017,4269251
