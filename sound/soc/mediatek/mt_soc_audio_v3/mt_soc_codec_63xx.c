...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.0912, add kcontrol for HAC control*/
static const char * const hac_switch_function[] = {"Off", "On" };
/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2016.0912*/
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.0912, add kcontrol for HAC control*/
int hac_switch_state = 0;
static int Hac_Switch_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	printk("%s: hac_switch_state = %d\n", __func__, hac_switch_state);
	ucontrol->value.integer.value[0] = hac_switch_state;

	return 0;
}

static int Hac_Switch_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int index = 0;

        if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(hac_switch_function)) {
                pr_err("return -EINVAL\n");
                return -EINVAL;
        }
        index = ucontrol->value.integer.value[0];
	...
	{
		printk("%s: Hac close!\n", __func__);
		mt_set_gpio_mode((123 | 0x80000000), GPIO_MODE_00);
        	mt_set_gpio_dir((123 | 0x80000000), GPIO_DIR_OUT);
		mt_set_gpio_out((123 | 0x80000000), GPIO_OUT_ZERO);
		hac_switch_state = 0;
	}
	else if (index == 1)
	{
		printk("%s: Hac open!\n", __func__);
                mt_set_gpio_mode((123 | 0x80000000), GPIO_MODE_00);
                mt_set_gpio_dir((123 | 0x80000000), GPIO_DIR_OUT);
                mt_set_gpio_out((123 | 0x80000000), GPIO_OUT_ONE);
		hac_switch_state = 1;
	}
	else
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.0912, add kcontrol for HAC control*/
        SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(hac_switch_function), hac_switch_function),
        /* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2016.0912*/
};
...
static const struct snd_kcontrol_new mt6331_snd_controls[] = {
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2016.0912, add kcontrol for HAC control*/
        SOC_ENUM_EXT("Hac_Switch", Audio_DL_Enum[12], Hac_Switch_Get, Hac_Switch_Set),
        /* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2016.0912*/
};
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0311, 0xff19);           /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
				/* Enable MICBIAS0, MISBIAS0 = 1P9V,
				...
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0310, 0xff90);           /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {	/* "ADC1", main_mic */
				...
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0301, 0xff09);		/* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
				...
				if (ECMmode == 1) {	/* differential */
					...
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0317, 0xff1f);	/* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
					...
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0313, 0xff1f); /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
					...
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0311, 0xff1f); /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
					/* Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V */
				}
				...
				if (ECMmode == 1) {	/* differential */
					...
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0370, 0xfff0);		/* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
					/* Enable MICBIAS1, SwithP/N MISBIAS1 = 2P5V */
				} else if (ECMmode == 2) {	/* single end */
					//Ana_Set_Reg(AUDENC_ANA_CON9, 0x0730, 0xfff0);
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0330, 0xfff0);           /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
					...
					Ana_Set_Reg(AUDENC_ANA_CON9, 0x0310, 0xfff0);           /* [PLATFORM]-Add by TCTSZ.lizhi.wu, 2016.09.20, change micbias from 2.5 to 2.0*/
...
/*[FEATURE]-add-begin.scdtablet jinghuang@tcl.com,2016.12.02,3649975*/
/*add stero speaker  for pixi5104g*/
#ifdef CONFIG_PROJECT_PIXI5104G
static const char * const Speaker_PA_R_function[] ={"Off","On"};
static const char * const Speaker_PA_L_function[] ={"Off","On"};
#endif
/*[FEATURE]-ADD-END*/
...
/*add stero speaker  for pixi5104g*/
#ifdef CONFIG_PROJECT_PIXI5104G
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Speaker_PA_R_function),Speaker_PA_R_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Speaker_PA_L_function),Speaker_PA_L_function),
#endif
/*[FEATURE]-ADD-END*/
};
...
/*add stero speaker  for pixi5104g*/
#ifdef CONFIG_PROJECT_PIXI5104G
static  int Speaker_R_switch_state = 0;
static int Speaker_PA_R_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
       printk("%s: Speaker_R_switch_state = %d\n", __func__, Speaker_R_switch_state);
       ucontrol->value.integer.value[0] = Speaker_R_switch_state;

       return 0;
}

static int Speaker_PA_R_switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
       int index = 0;

        if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Speaker_PA_R_function)) {
                pr_err("return -EINVAL\n");
                return -EINVAL;
        }
        index = ucontrol->value.integer.value[0];
       ...
       if (index == 0)
       {
               printk("%s: Speaker_R_switch_state close!\n", __func__);
               mt_set_gpio_mode((125 | 0x80000000), GPIO_MODE_00);
               mt_set_gpio_dir((125 | 0x80000000), GPIO_DIR_OUT);
               mt_set_gpio_out((125 | 0x80000000), GPIO_OUT_ZERO);
               Speaker_R_switch_state = 0;
       }
       else if (index == 1)
       {
               printk("%s: Speaker_R_switch_state open!\n", __func__);
                mt_set_gpio_mode((125 | 0x80000000), GPIO_MODE_00);
                mt_set_gpio_dir((125 | 0x80000000), GPIO_DIR_OUT);
                mt_set_gpio_out((125 | 0x80000000), GPIO_OUT_ONE);
		udelay(1);
		mt_set_gpio_out((125 | 0x80000000), GPIO_OUT_ZERO);
		udelay(1);
		mt_set_gpio_out((125 | 0x80000000), GPIO_OUT_ONE);
...
static  int Speaker_L_switch_state = 0;
static int Speaker_PA_L_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
       printk("%s: Speaker_L_switch_state = %d\n", __func__, Speaker_L_switch_state);
       ucontrol->value.integer.value[0] = Speaker_L_switch_state;

       return 0;
}

static int Speaker_PA_L_switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
       int index = 0;

        if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Speaker_PA_L_function)) {
                pr_err("return -EINVAL\n");
                return -EINVAL;
        }
        index = ucontrol->value.integer.value[0];
       ...
       if (index == 0)
       {
               printk("%s: Speaker_L_switch_state close!\n", __func__);
               mt_set_gpio_mode((124 | 0x80000000), GPIO_MODE_00);
               mt_set_gpio_dir((124 | 0x80000000), GPIO_DIR_OUT);
               mt_set_gpio_out((124 | 0x80000000), GPIO_OUT_ZERO);
               Speaker_L_switch_state = 0;
       }
       else if (index == 1)
       {
               printk("%s: Speaker_L_switch_state open!\n", __func__);
                mt_set_gpio_mode((124 | 0x80000000), GPIO_MODE_00);
                mt_set_gpio_dir((124 | 0x80000000), GPIO_DIR_OUT);
                mt_set_gpio_out((124 | 0x80000000), GPIO_OUT_ONE);
		udelay(1);
                mt_set_gpio_out((124 | 0x80000000), GPIO_OUT_ZERO);
                udelay(1);
                mt_set_gpio_out((124 | 0x80000000), GPIO_OUT_ONE);
...
/*add stero speaker  for pixi5104g*/
#ifdef CONFIG_PROJECT_PIXI5104G
    SOC_ENUM_EXT("Speaker_PA_R_Switch",Audio_UL_Enum[26],Speaker_PA_R_switch_get,Speaker_PA_R_switch_set),
    SOC_ENUM_EXT("Speaker_PA_L_Switch",Audio_UL_Enum[27],Speaker_PA_L_switch_get,Speaker_PA_L_switch_set),
    #endif
/*[FEATURE]-ADD-END*/
};
